...
$sq_settings = mysqli_fetch_assoc(mysqlQuery("select * from app_settings where setting_id='1'"));
...
if($tax_type=='Monthly' && $tax_pay_day==$tax_day){
	$sq_count = mysqli_num_rows(mysqlQuery("SELECT * from  remainder_status where remainder_name = 'tax_pay_raminder' and date='$today' and status='Done'"));
	if($sq_count==0){
	...
	$max=$value['max']+1;
	$sq_check_status=mysqlQuery("INSERT INTO `remainder_status`(`id`, `remainder_name`, `date`, `status`) VALUES ('$max','tax_pay_raminder','$today','Done')");
}
