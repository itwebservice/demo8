...
if($day == 19 && ($month == "January" || $month == "April"  || $month == "July"  || $month == "October" )){
    ...
    $sq_count = mysqli_num_rows(mysqlQuery("SELECT * from  remainder_status where remainder_name = 'hotel_tarrif_remainder' and date='$cur_date' and status='Done'"));
            if($sq_count==0)
            {
...
$max=$value['max']+1;
$sq_check_status=mysqlQuery("INSERT INTO `remainder_status`(`id`, `remainder_name`, `date`, `status`) VALUES ('$max','hotel_tarrif_remainder','$cur_date','Done')");
