...
global $model;
	...
	$sq_emplyee = mysqlQuery("SELECT * from emp_master where expiry_date='$exp_date' and active_flag='Active'");
	while($row_emp=mysqli_fetch_assoc($sq_emplyee))
	{
		...
		$sq_count = mysqli_num_rows(mysqlQuery("SELECT * from  remainder_status where remainder_name = 'user_visa_renewal' and date='$today' and status='Done'"));
		if($sq_count==0){
	...
	if($sq_count==0){
		...
		$max=$value['max']+1;
		$sq_check_status=mysqlQuery("INSERT INTO `remainder_status`(`id`, `remainder_name`, `date`, `status`) VALUES ('$max','user_visa_renewal','$today','Done')");
	}
