...

{
    ...
    $sq_booking = mysqli_fetch_assoc(mysqlQuery("select customer_id, taxation_type from tourwise_traveler_details where id='$tourwise_id'"));
    ...
    $sq_max = mysqli_fetch_assoc(mysqlQuery("select max(estimate_id) as max from refund_tour_estimate"));
    $estimate_id = $sq_max['max'] + 1;

    $sq_est = mysqlQuery("insert into refund_tour_estimate(estimate_id, tourwise_traveler_id, cancel_amount, total_refund_amount, created_at) values ('$estimate_id', '$tourwise_id', '$cancel_amount', '$total_refund_amount', '$created_at')");       
