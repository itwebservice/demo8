...
public function cancel_tour_group($tour_id, $tour_group_id)
{
  ...
  { 
    $sq1 = mysqlQuery("update tour_groups set status='Cancel' where tour_id='$tour_id' and group_id='$tour_group_id[$i]' ");
    if(!$sq1){
      $GLOBALS['flag'] = false;
    }

    $sq2 = mysqlQuery("update tourwise_traveler_details set tour_group_status='Cancel' where tour_id='$tour_id' and tour_group_id='$tour_group_id[$i]' ");
    if(!$sq2){
      $GLOBALS['flag'] = false;
    }
  } 
...
public function refund_tour_group_fee_save()
{
  ...
  if($refund_mode == 'Credit Note'){
    ...
    $sq_payment = mysqlQuery("insert into credit_note_master (id, financial_year_id, module_name, module_entry_id, customer_id, payment_amount,refund_id,created_at,branch_admin_id) values ('$id', '$financial_year_id', 'Group Booking', '$tourwise_id', '$customer_id','$refund_amount','$max2','$refund_date','$branch_admin_id') ");
  }
  if(!$sq)
  {
    rollback_t();
...
function extra_travel_amount_refund_status_update()
{
  ...
  $refund_id = mysqli_fetch_assoc(mysqlQuery("select max(refund_id) as max from refund_travel_extra_amount"));
  $refund_id = $refund_id['max']+1;

  $sq_refund = mysqlQuery("insert into refund_travel_extra_amount ( refund_id, tourwise_traveler_id, refund_amount, refund_mode, refund_date ) values ( '$refund_id', '$tourwise_id', '$refund_amount', '$refund_mode', '$refund_date' )");
  if($sq_refund)
  {  
