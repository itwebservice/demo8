...
Class AirFiles {
    ...
    public function saveGDS($arrGDS, $arrTicket, $arrno_tkt, $arrPax, $type){
      ...
      for ($i = 0; $i < sizeof($arrGDS); $i++) { 
        ...
        $sq_max = mysqli_fetch_assoc(mysqlQuery("SELECT max(ticket_airfile_id) as max from ticket_master_airfile"));
        $ticket_id = $sq_max['max'] + 1;

        $ticket_master = mysqlQuery("INSERT INTO `ticket_master_airfile` (ticket_airfile_id, total_cost, basic_cost,yq_tax, other_taxes, issue_date) VALUES ('$ticket_id', '$totalCost', '$basicFare','$yq','$other_taxes','$bookDate' ) ");
        ...
        for ($j = 1; $j <= sizeof($arrPax[$i]); $j++){
          ...
          $sq_max = mysqli_fetch_assoc(mysqlQuery("select max(entry_id) as max from ticket_master_entries_airfile"));
          $entry_id = $sq_max['max'] + 1;
          $sq_entry = mysqlQuery("INSERT INTO ticket_master_entries_airfile(`entry_id`, `ticket_airfile_id`, `gds_pnr`, `first_name`, `middle_name`, `last_name`, `adolescence`,`ticket_no`,`cticket_no`,`file_type` ) VALUES ('$entry_id','$ticket_id', '$pnr_no', '$first_name','$middle_name','$last_name', '$adolescence','$ticket_no','$cticket_no','$type')");
        ...
        for ($j = 1; $j <= sizeof($arrGDS[$i]); $j++){
          ...
          $airline_name = mysqli_fetch_assoc(mysqlQuery("SELECT `airline_name` FROM `airline_master` WHERE   `active_flag` = 'Active' AND `airline_code` = '".$arrGDS[$i][$j]['Airline']."'"));
          ...
          $from_sector = mysqli_fetch_assoc(mysqlQuery("SELECT `city_id`, `airport_name`, `airport_code` FROM `airport_master` WHERE `flag` = 'Active' AND `airport_code` = '".$arrGDS[$i][$j]['Sector_From']."'"));
          $to_sector = mysqli_fetch_assoc(mysqlQuery("SELECT `city_id`, `airport_name` FROM `airport_master` WHERE `flag` = 'Active' AND `airport_code` = '".$arrGDS[$i][$j]['Sector_To']."'"));
          ...
          $sq_max = mysqli_fetch_assoc(mysqlQuery("select max(entry_id) as max from ticket_trip_entries_airfile"));
          $entry_id = $sq_max['max'] + 1;
          $sq_entry = mysqlQuery("INSERT INTO ticket_trip_entries_airfile(`entry_id`, `ticket_airfile_id`, `departure_datetime`, `arrival_datetime`, `airlines_name`, `class`, `flight_no`, `from_city`, `to_city`, `departure_city`, `arrival_city`) VALUES('$entry_id', '$ticket_id', '$departDateTime', '$arrDateTime', '$airline', '$class', '$flight_no', '$from_city', '$to_city', '$from_airp', '$to_airp')");
          if(!$sq_entry){
            $cflag = false;
          }
        }
    ...
    public function getCurrency(){
      ...
      $currency_logo_d = mysqli_fetch_assoc(mysqlQuery("SELECT `currency_code` FROM `currency_name_master` WHERE id=".$currency));
