...
class credit_card{
	...
	public function save(){
		...
		$sq_credit = mysqli_num_rows(mysqlQuery("SELECT `company_name` FROM `credit_card_company` WHERE company_name='$company_name'"));
		if($sq_credit > 0){
			echo "error--Credit card company name already exist!";
			exit;
		}

		$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(entry_id) as max from credit_card_company"));
		$entry_id = $sq_max['max'] + 1;

		$sq_transaction = mysqlQuery("INSERT INTO `credit_card_company`(`entry_id`, `company_name`, `charges_in`, `credit_card_charges`, `tax_charges_in`, `tax_on_credit_card_charges`,`bank_id`, `membership_details_arr`, `created_at`,`status`) VALUES ('$entry_id','$company_name','$charges_in','$credit_card_charges','$tax_charges_in','$tax_on_credit_card_charges','$bank_id','$membership_details_arr','$created_at','Active')");

		$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(ledger_id) as max from ledger_master"));
		$ledger_id = $sq_max['max'] + 1;
		$sq_ledger = mysqlQuery("insert into ledger_master (ledger_id, ledger_name, alias, group_sub_id, balance, dr_cr,customer_id,user_type,status) values ('$ledger_id', '$company_name', '', '20', '0','Dr','$entry_id','credit company','Active')");
