...
public function package_tour_checklist_save()
{
	...
	$sq_del_checklist = mysqlQuery("delete from checklist_package_tour where booking_id='$booking_id'");
	if(!$sq_del_checklist){
	...
	for($i=0; $i<sizeof($entity_id_arr); $i++){
		...
		$sq_checklist = mysqlQuery("insert into checklist_package_tour( id, branch_admin_id, booking_id, tour_type,entity_id ) values ( '$id', '$branch_admin_id', '$booking_id','$tour_type' ,'$entity_id_arr[$i]' )");
		if(!$sq_checklist){
