...

{
    ...
    $att_query = mysqli_num_rows(mysqlQuery("select * from employee_attendance_log where emp_id='$emp_id_arr[$i]' and att_date='$att_date'"));
 
    $sq_max = mysqli_fetch_assoc(mysqlQuery("select max(attendance_id) as max from employee_attendance_log"));

	$attendance_id = $sq_max['max'] + 1;

    	if($att_query == 0){

    		    $sq_log = mysqlQuery("insert into employee_attendance_log ( attendance_id, emp_id, att_date, status ) values ( '$attendance_id',  '$emp_id_arr[$i]', '$att_date', '$status_arr[$i]' )");
    		    
    	}
    	else
    	{
    		 
    		   	$sq_log = mysqlQuery("update employee_attendance_log set status='$status_arr[$i]' where att_date='$att_date' and  emp_id='$emp_id_arr[$i]'");
    	}
    ...
    if(empty($handle) === false) {
        ...
        while(($data = fgetcsv($handle,3000, ",")) !== FALSE){
            ...
            {                
			    ...
			    $att_query = mysqli_num_rows(mysqlQuery("select * from employee_attendance_log where month='$month' and year='$year' and emp_id='$emp_id'"));
				...
				for($i=0; $i<sizeof($log_id_arr); $i++){
					...
					if($log_id_arr[$i]!=""){
						...
						$sq_log = mysqlQuery("update user_logs set status='$status_arr[$i]', remark='$remark1' where log_id='$log_id_arr[$i]'");
					    ...
					    $sq_log = mysqlQuery("insert into user_logs ( log_id, login_id, login_date, status, remark ) values ( '$log_id', '$login_id', '$login_date_arr[$i]', '$status_arr[$i]', '$remark1' )");
