...
class miscellaneous_payment_master
{
	...
	public function finance_save($payment_id, $branch_admin_id)
	{
		...
		if ($payment_mode != 'Credit Note') {
			...
			if ($payment_mode == 'Credit Card') {
				...
				$payment_date = $payment_date;
				$payment_particular = get_sales_paid_particular(get_misc_booking_payment_id($misc_id, $yr1), $payment_date, $finance_charges, $customer_id, $payment_mode, get_misc_booking_id($misc_id, $yr1), $bank_id1, $transaction_id1);
				$ledger_particular = get_ledger_particular('By', 'Cash/Bank');
				$gl_id = 231;
				$payment_side = "Debit";
				$clearance_status = ($payment_mode == "Cheque" || $payment_mode == "Credit Card") ? "Pending" : "";
	...
	public function finance_update($sq_payment_info, $clearance_status1)
	{
		...
		if ($payment_amount1 != $payment_old_value) {
			...
			if ($payment_mode == 'Credit Card') {
				...
				$payment_date = $payment_date;
				$payment_particular = get_sales_paid_particular(get_misc_booking_payment_id($misc_id, $yr1), $payment_date, $credit_charges_old, $customer_id, $payment_mode, get_misc_booking_id($misc_id, $yr1), $bank_id, $transaction_id1);
				$ledger_particular = get_ledger_particular('By', 'Cash/Bank');
				$gl_id = $cust_gl;
				$payment_side = "Credit";
				$clearance_status = ($payment_mode == "Cheque" || $payment_mode == "Credit Card") ? "Pending" : "";
				...
				$payment_amount = $finance_charges;
				$payment_date = $payment_date;
				$payment_particular = get_sales_paid_particular(get_misc_booking_payment_id($misc_id, $yr1), $payment_date, $finance_charges, $customer_id, $payment_mode, get_misc_booking_id($misc_id, $yr1), $bank_id, $transaction_id1);
				$ledger_particular = get_ledger_particular('By', 'Cash/Bank');
				$gl_id = 231;
				$payment_side = "Credit";
				$clearance_status = ($payment_mode == "Cheque" || $payment_mode == "Credit Card") ? "Pending" : "";
				...
				$payment_amount = $credit_company_amount;
				$payment_date = $payment_date;
				$payment_particular = get_sales_paid_particular(get_misc_booking_payment_id($misc_id, $yr1), $payment_date, $credit_company_amount, $customer_id, $payment_mode, get_misc_booking_id($misc_id, $yr1), $bank_id, $transaction_id1);
				$ledger_particular = get_ledger_particular('By', 'Cash/Bank');
				$gl_id = $company_gl;
				$payment_side = "Credit";
				$clearance_status = ($payment_mode == "Cheque" || $payment_mode == "Credit Card") ? "Pending" : "";
				...
				$payment_date = $payment_date;
				$payment_particular = get_sales_paid_particular(get_misc_booking_payment_id($misc_id, $yr1), $payment_date, $payment_old_value, $customer_id, $payment_mode, get_misc_booking_id($misc_id, $yr1), $bank_id, $transaction_id1);
				$ledger_particular = get_ledger_particular('By', 'Cash/Bank');
				$gl_id = $pay_gl;
				$payment_side = "Credit";
				$clearance_status = ($payment_mode == "Cheque" || $payment_mode == 'Credit Card') ? "Pending" : "";
	...
	public function payment_email_notification_send($misc_id, $payment_amount, $payment_mode, $payment_date)
	{
		...
		global $model;
		$model->generic_payment_mail('97', $payment_amount, $payment_mode, $total_amount, $total_pay_amt, $payment_date, $due_date, $email_id, $subject, $customer_name);
	}
	...
	{
		global $secret_key, $encrypt_decrypt;
		$sq_visa_info = mysqli_fetch_assoc(mysqlQuery("select customer_id from miscellaneous_master where misc_id='$visa_id'"));
		$customer_id = $sq_visa_info['customer_id'];
	...
	public function whatsapp_send()
	{
		...
		$sq_pay = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum,sum(`credit_charges`) as sumc from miscellaneous_payment_master where clearance_status!='Cancelled' and misc_id=" . $_POST['booking_id']));
