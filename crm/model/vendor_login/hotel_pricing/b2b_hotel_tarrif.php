...
public function hotel_tarrif_save(){
	...
	$tab2_from_date_array = ($_POST['tab2_from_date_array']!='')?$_POST['tab2_from_date_array']:[];
	...
	$tab3_from_date_array = ($_POST['tab3_from_date_array']!='')?$_POST['tab3_from_date_array']:[];
	...
	$tab4_room_cat_array = ($_POST['tab4_room_cat_array']!='')?$_POST['tab4_room_cat_array']:[];
	...
	$sq_hotel = mysqlQuery("update hotel_master set taxation_type='$taxation_type', taxation_id='$taxation_id',service_tax='$service_tax' where city_id='$city_id' and hotel_id='$hotel_id'");
	$sq_login = mysqlQuery("insert into hotel_vendor_price_master (pricing_id, city_id,hotel_id, currency_id , created_at) values ('$pricing_id','$city_id','$hotel_id','$currency_id', '$created_at')");
	if($sq_login){
		//TAB-2
		for($i=0; $i<sizeof($tab2_from_date_array); $i++){

			$sq = mysqlQuery("select max(entry_id) as max from hotel_contracted_tarrif");
			$value = mysqli_fetch_assoc($sq);
			$max_entry_id = $value['max'] + 1;
			
			$tab2_room_cat_array[$i] = mysqlREString($tab2_room_cat_array[$i]);
			$tab2_max_occ_array[$i] = mysqlREString($tab2_max_occ_array[$i]);
			$tab2_from_date_array[$i] = mysqlREString($tab2_from_date_array[$i]);
			$tab2_to_date_array[$i] = mysqlREString($tab2_to_date_array[$i]);
			$tab2_double_bed_array[$i] = mysqlREString($tab2_double_bed_array[$i]);
			$tab2_chwithbed_array[$i] = mysqlREString($tab2_chwithbed_array[$i]);
			$tab2_chwobed_array[$i] = mysqlREString($tab2_chwobed_array[$i]);
			$tab2_extra_bed_array[$i] = mysqlREString($tab2_extra_bed_array[$i]);
			$tab2_markup_per_array[$i] = mysqlREString($tab2_markup_per_array[$i]);
			$tab2_markup_cost_array[$i] = mysqlREString($tab2_markup_cost_array[$i]);
			$tab2_meal_plan_array[$i] = mysqlREString($tab2_meal_plan_array[$i]);
		...
		for($i=0; $i<sizeof($tab3_from_date_array); $i++){
			...
			$sq = mysqlQuery("select max(entry_id) as max from hotel_blackdated_tarrif");
			$value = mysqli_fetch_assoc($sq);
			$max_entry_id = $value['max'] + 1;
			
			$tab3_room_cat_array[$i] = mysqlREString($tab3_room_cat_array[$i]);
			$tab3_max_occ_array[$i] = mysqlREString($tab3_max_occ_array[$i]);
			$tab3_from_date_array[$i] = mysqlREString($tab3_from_date_array[$i]);
			$tab3_to_date_array[$i] = mysqlREString($tab3_to_date_array[$i]);
			$tab3_double_bed_array[$i] = mysqlREString($tab3_double_bed_array[$i]);
			$tab3_chwithbed_array[$i] = mysqlREString($tab3_chwithbed_array[$i]);
			$tab3_chwobed_array[$i] = mysqlREString($tab3_chwobed_array[$i]);
			$tab3_extra_bed_array[$i] = mysqlREString($tab3_extra_bed_array[$i]);
			$tab3_markup_per_array[$i] = mysqlREString($tab3_markup_per_array[$i]);
			$tab3_markup_cost_array[$i] = mysqlREString($tab3_markup_cost_array[$i]);
			$tab3_meal_plan_array[$i] = mysqlREString($tab3_meal_plan_array[$i]);
		...
		for($i=0; $i<sizeof($tab4_room_cat_array); $i++){
			...
			$sq = mysqlQuery("select max(entry_id) as max from hotel_weekend_tarrif");
			$value = mysqli_fetch_assoc($sq);
			$max_entry_id = $value['max'] + 1;

			$tab4_room_cat_array[$i] = mysqlREString($tab4_room_cat_array[$i]);
			$tab4_max_occ_array[$i] = mysqlREString($tab4_max_occ_array[$i]);
			$tab4_day_array[$i] = mysqlREString($tab4_day_array[$i]);
			$tab4_double_bed_array[$i] = mysqlREString($tab4_double_bed_array[$i]);
			$tab4_chwithbed_array[$i] = mysqlREString($tab4_chwithbed_array[$i]);
			$tab4_chwobed_array[$i] = mysqlREString($tab4_chwobed_array[$i]);
			$tab4_extra_bed_array[$i] = mysqlREString($tab4_extra_bed_array[$i]);
			$tab4_markup_per_array[$i] = mysqlREString($tab4_markup_per_array[$i]);
			$tab4_markup_cost_array[$i] = mysqlREString($tab4_markup_cost_array[$i]);
			$tab4_meal_plan_array[$i] = mysqlREString($tab4_meal_plan_array[$i]);
	
			$sq1 = mysqlQuery("INSERT INTO `hotel_weekend_tarrif`(`entry_id`, `pricing_id`, `room_category`, `day`,  `double_bed`, `child_with_bed`, `child_without_bed`, `extra_bed`, `markup_per`, `markup`, `meal_plan`,`max_occupancy`) VALUES ('$max_entry_id','$pricing_id','$tab4_room_cat_array[$i]','$tab4_day_array[$i]','$tab4_double_bed_array[$i]','$tab4_chwithbed_array[$i]','$tab4_chwobed_array[$i]','$tab4_extra_bed_array[$i]','$tab4_markup_per_array[$i]','$tab4_markup_cost_array[$i]','$tab4_meal_plan_array[$i]','$tab4_max_occ_array[$i]')");
			if(!$sq1){
				$GLOBALS['flag'] = false;
		...
		for($i=0; $i<sizeof($from_date_array); $i++){
			...
			$sq = mysqlQuery("select max(entry_id) as max from hotel_offers_tarrif");
			$value = mysqli_fetch_assoc($sq);
			$max_entry_id = $value['max'] + 1;
			...
			$sq1 = mysqlQuery("INSERT INTO `hotel_offers_tarrif`(`entry_id`, `hotel_id`,`type`, `from_date`, `to_date`, `offer`,`offer_amount`,`coupon_code`, `agent_type`) VALUES ('$max_entry_id','$hotel_id','$type_array[$i]','$from_date_array[$i]','$to_date_array[$i]','$offer_array[$i]','$offer_amount_array[$i]','$coupon_code_array[$i]','$agent_array[$i]')");
			if(!$sq1){
				$GLOBALS['flag'] = false;
...
public function hotel_tarrif_update(){
	...
	$sq_hotel = mysqlQuery("update hotel_master set taxation_type='$taxation_type', taxation_id='$taxation_id',service_tax='$service_tax' where city_id='$city_id' and hotel_id='$hotel_id'");
	$sq_login = mysqlQuery("update hotel_vendor_price_master set city_id='$city_id',hotel_id='$hotel_id', currency_id='$currency_id' where pricing_id='$pricing_id'");
	if($sq_login){
		//TAB-2
		for($i=0; $i<sizeof($tab2_from_date_array); $i++){
			if($tab2_checked_id_array[$i] == 'true'){
				...
				if($tab2_entry_id_array[$i] == ''){
					...
					$sq1 = mysqlQuery("UPDATE `hotel_contracted_tarrif` SET `room_category`='$tab2_room_cat_array[$i]',`from_date`='$tab2_from_date_array[$i]',`to_date`='$tab2_to_date_array[$i]',`double_bed`='$tab2_double_bed_array[$i]',`child_with_bed`='$tab2_chwithbed_array[$i]',`child_without_bed`='$tab2_chwobed_array[$i]',`extra_bed`='$tab2_extra_bed_array[$i]',`markup_per`='$tab2_markup_per_array[$i]',`markup`='$tab2_markup_cost_array[$i]',`meal_plan`='$tab2_meal_plan_array[$i]',`max_occupancy`='$tab2_max_occ_array[$i]' WHERE entry_id='$tab2_entry_id_array[$i]'");
				}
			}
			else{
				$sq1 = mysqlQuery("DELETE FROM `hotel_contracted_tarrif` WHERE `entry_id`='$tab2_entry_id_array[$i]'");
			}
			if(!$sq1){
				$GLOBALS['flag'] = false;
				echo "error--Hotel Seasonal Tariff details not updated!";
			}
		}
		//TAB-3
		for($i=0; $i<sizeof($tab3_from_date_array); $i++){
		...
		for($i=0; $i<sizeof($tab4_room_cat_array); $i++){
			if($tab4_checked_id_array[$i] == 'true'){
				...
				if($tab4_entry_id_array[$i] == ''){
					...
					$tab4_max_occ_array[$i] = mysqlREString($tab4_max_occ_array[$i]);
					$tab4_day_array[$i] = mysqlREString($tab4_day_array[$i]);
					$tab4_single_bed_array[$i] = mysqlREString($tab4_single_bed_array[$i]);
					$tab4_double_bed_array[$i] = mysqlREString($tab4_double_bed_array[$i]);
					$tab4_triple_bed_array[$i] = mysqlREString($tab4_triple_bed_array[$i]);
					$tab4_chwithbed_array[$i] = mysqlREString($tab4_chwithbed_array[$i]);
					$tab4_chwobed_array[$i] = mysqlREString($tab4_chwobed_array[$i]);
					$tab4_fchild_array[$i] = mysqlREString($tab4_fchild_array[$i]);
					$tab4_schild_array[$i] = mysqlREString($tab4_schild_array[$i]);
					$tab4_extra_bed_array[$i] = mysqlREString($tab4_extra_bed_array[$i]);
					$tab4_queen_bed_array[$i] = mysqlREString($tab4_queen_bed_array[$i]);
					$tab4_king_bed_array[$i] = mysqlREString($tab4_king_bed_array[$i]);
					$tab4_quad_bed_array[$i] = mysqlREString($tab4_quad_bed_array[$i]);
					$tab4_twin_bed_array[$i] = mysqlREString($tab4_twin_bed_array[$i]);
					$tab4_markup_per_array[$i] = mysqlREString($tab4_markup_per_array[$i]);
					$tab4_markup_cost_array[$i] = mysqlREString($tab4_markup_cost_array[$i]);
					$tab4_meal_plan_array[$i] = mysqlREString($tab4_meal_plan_array[$i]);
					...
					$sq1 = mysqlQuery("UPDATE `hotel_weekend_tarrif` SET `room_category`='$tab4_room_cat_array[$i]',`day`='$tab4_day_array[$i]',`double_bed`='$tab4_double_bed_array[$i]',`child_with_bed`='$tab4_chwithbed_array[$i]',`child_without_bed`='$tab4_chwobed_array[$i]',`extra_bed`='$tab4_extra_bed_array[$i]',`markup_per`='$tab4_markup_per_array[$i]',`markup`='$tab4_markup_cost_array[$i]',`meal_plan`='$tab4_meal_plan_array[$i]',`max_occupancy`='$tab4_max_occ_array[$i]' WHERE entry_id='$tab4_entry_id_array[$i]'");
				}
			}
			else{
				$sq1 = mysqlQuery("DELETE FROM `hotel_weekend_tarrif` WHERE `entry_id`='$tab4_entry_id_array[$i]'");
			}
			if(!$sq1){
				$GLOBALS['flag'] = false;
		...
		for($i=0; $i<sizeof($from_date_array); $i++){
			if($tab5_checked_id_array[$i] == 'true'){
				...
				if($tab5_entry_id_array[$i] == ''){
					...
					if($from_date_array[$i]!=""){  $from_date_array[$i] = date("Y-m-d", strtotime($from_date_array[$i])); }
					if($to_date_array[$i]!=""){  $to_date_array[$i] = date("Y-m-d", strtotime($to_date_array[$i])); }
					$sq1 = mysqlQuery("UPDATE `hotel_offers_tarrif` SET  `type`='$type_array[$i]',`from_date`='$from_date_array[$i]', `to_date`='$to_date_array[$i]', `offer`='$offer_array[$i]', `agent_type`='$agent_array[$i]',`coupon_code`='$coupon_code_array[$i]',`offer_amount`='$offer_amount_array[$i]' WHERE entry_id='$tab5_entry_id_array[$i]'");
				}
			}
			else{
				$sq1 = mysqlQuery("DELETE FROM `hotel_offers_tarrif` WHERE `entry_id`='$tab5_entry_id_array[$i]'");
			}
			if(!$sq1){
				$GLOBALS['flag'] = false;
