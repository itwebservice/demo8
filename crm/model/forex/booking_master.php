...
public function booking_mail($booking_id, $customer_id){
    ...
    $sq_visa = mysqli_fetch_assoc(mysqlQuery("select * from  forex_booking_master where booking_id='$booking_id'"));
    $date = $sq_visa['created_at'];
    $yr = explode("-", $date);
    $year =$yr[0];
    $sq_pay = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum,sum(`credit_charges`) as sumc from forex_booking_payment_master where clearance_status!='Cancelled' and booking_id='$booking_id'"));
    $credit_card_amount = $sq_pay['sumc'];
    $total_amount = $sq_visa['net_total']+$credit_card_amount;
...
public function finance_update($sq_booking_info,$particular)
{
    ...
    $sq_forex = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as payment_amount from forex_booking_payment_master where booking_id='$booking_id'"));
