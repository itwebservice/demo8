...
public function hotel_inventory_update(){
	...
	$reminder1 = date("Y-m-d", strtotime($reminder1));
	$reminder2 = date("Y-m-d", strtotime($reminder2));
	$sq_count = mysqli_num_rows(mysqlQuery("select * from hotel_inventory_master where city_id='$city_id' and hotel_id='$hotel_name' and room_type='$room_type' and valid_from_date='$from_date' and valid_to_date='$to_date' and entry_id!='$entry_id'"));
	if($sq_count != 0){
		echo "error--Sorry, Inventory already added!";
		exit;
	}

	$sq_service = mysqlQuery("update hotel_inventory_master set purchase_date='$purchase_date', room_type='$room_type',total_rooms='$no_of_rooms',valid_from_date='$from_date', valid_to_date='$to_date',rate='$rate',cancel_date='$cancel_date', reminder1='$reminder1' , reminder2='$reminder2',note='$note',active_flag='$active_flag' where entry_id='$entry_id'");
