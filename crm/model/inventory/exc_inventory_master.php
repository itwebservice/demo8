...
public function exc_inventory_save()
{
	...
	$sq_count = mysqli_num_rows(mysqlQuery("select * from excursion_inventory_master where city_id='$city_id' and exc_id='$exc_name' and valid_from_date='$from_date' and valid_to_date='$to_date'"));
	if($sq_count != 0){
		echo "error--Sorry, Inventory already added!";
		exit;
	}
	$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(entry_id) as max from excursion_inventory_master"));
	$service_id = $sq_max['max'] + 1;

	$sq_service = mysqlQuery("insert into excursion_inventory_master (entry_id,emp_id,branch_admin_id,financial_year_id, city_id, exc_id,purchase_date,total_tickets,valid_from_date, valid_to_date,rate,cancel_date, reminder1 , reminder2,note,created_at,active_flag) values ('$service_id','$emp_id','$branch_admin_id','$financial_year_id' , '$city_id', '$exc_name','$purchase_date','$total_tickets','$from_date','$to_date','$rate','$cancel_date','$reminder1' , '$reminder2','$note','$created_at','Active')");
...
public function exc_inventory_update(){
	...
	$sq_service = mysqlQuery("update excursion_inventory_master set purchase_date='$purchase_date', total_tickets='$total_tickets',valid_from_date='$from_date', valid_to_date='$to_date',rate='$rate',cancel_date='$cancel_date', reminder1='$reminder1' , reminder2='$reminder2',note='$note',active_flag='$active_flag' where entry_id='$entry_id'");
