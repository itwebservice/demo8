...
public function ticket_cancel_save()
{
	...
	for($i=0; $i<sizeof($entry_id_arr); $i++){
		$sq_cancel = mysqlQuery("update train_ticket_master_entries set status='Cancel' where entry_id='$entry_id_arr[$i]'");
		if(!$sq_cancel){
...
public function cancel_mail_send($entry_id_arr){
	...
	for($i=0; $i<sizeof($entry_id_arr); $i++){
	$sq_entry = mysqli_fetch_assoc(mysqlQuery("select * from train_ticket_master_entries where entry_id='$entry_id_arr[$i]'"));
	...
	$model->app_email_send('34',$cust_name,$email_id, $content,$subject);
}
