...

{
	...
	for($i=0; $i<sizeof($entry_id_arr); $i++){
		...
		$sq_cancel = mysqlQuery("update visa_master_entries set status='Cancel' where entry_id='$entry_id_arr[$i]'");
	...
	echo "Visa Booking has been successfully cancelled.";
...

{
	...
	$sq_entry = mysqli_fetch_assoc(mysqlQuery("select * from visa_master_entries where entry_id='$entry_id_arr[0]'"));

	$sq_visa_info = mysqli_fetch_assoc(mysqlQuery("select * from visa_master where visa_id='$sq_entry[visa_id]'"));
	...
	$sq_entry = mysqli_fetch_assoc(mysqlQuery("select * from visa_master_entries where entry_id='$entry_id_arr[$i]'"));
	...
	$model->app_email_send('30',$cust_name,$email_id, $content,$subject);
