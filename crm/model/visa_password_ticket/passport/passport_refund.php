...
public function passport_refund_save()
{
	...
	$sq_payment = mysqlQuery("insert into passport_refund_master (refund_id, passport_id, financial_year_id, refund_date, refund_amount, refund_mode, bank_name, transaction_id, bank_id, clearance_status, created_at) values ('$refund_id', '$passport_id', '$financial_year_id', '$refund_date', '$refund_amount', '$refund_mode', '$bank_name', '$transaction_id', '$bank_id', '$clearance_status', '$created_at') ");
	...
	if($refund_mode == 'Credit Note'){
		...
		$sq_payment = mysqlQuery("insert into credit_note_master (id, financial_year_id, module_name, module_entry_id, customer_id, payment_amount, refund_id, created_at,branch_admin_id) values ('$id', '$financial_year_id', 'Passport Booking', '$passport_id', '$customer_id','$refund_amount','$refund_id','$refund_date','$branch_admin_id') ");
	}
	...
	if(!$sq_payment){
		...
		for($i=0; $i<sizeof($entry_id_arr); $i++){
			...
			$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(id) as max from passport_refund_entries"));
			$id= $sq_max['max'] + 1;

			$sq_entry = mysqlQuery("insert into passport_refund_entries(id, refund_id, entry_id) values ('$id', '$refund_id', '$entry_id_arr[$i]')");
			if(!$sq_entry){
				$GLOBALS['glag'] = false;
...
public function refund_mail_send($passport_id,$refund_amount,$refund_date,$refund_mode,$transaction_id){
  ...
  $yr = explode("-", $date);
  $year =$yr[0];
  $cust_email = mysqli_fetch_assoc(mysqlQuery("select * from customer_master where customer_id='$sq_passport_info[customer_id]'"));
  $email_id = $encrypt_decrypt->fnDecrypt($cust_email['email_id'], $secret_key);

  $sq_paid_amount = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum from passport_payment_master where passport_id='$passport_id' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
  $sq_refund_amount1 = mysqli_fetch_assoc(mysqlQuery("select sum(refund_amount) as sum from passport_refund_master where passport_id='$passport_id' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
  $sq_pay=mysqli_fetch_assoc(mysqlQuery("select sum(refund_amount) as sum from passport_refund_master where passport_id='$passport_id'"));
  $refund_amount1 = ($sq_pay['sum'] == '')?'0':$sq_pay['sum'];

  $sq_pend_pay=mysqli_fetch_assoc(mysqlQuery("select sum(refund_amount) as sum from passport_refund_master where passport_id='$passport_id' and clearance_status='Pending'"));
  $pend_amount = ($sq_pend_pay['sum'] == '')?'0':$sq_pend_pay['sum'];

  $sq_canl_pay=mysqli_fetch_assoc(mysqlQuery("select sum(refund_amount) as sum from passport_refund_master where passport_id='$passport_id' and clearance_status='Cancelled'"));
  ...
  $cancel_amount = $sq_passport_info['cancel_amount'];
	...
	<tr><td style="text-align:left;border: 1px solid #888888;">Service Type</td>   <td style="text-align:left;border: 1px solid #888888;">Passport Booking</td></tr>
  ...
  $subject = 'Passport Cancellation Refund ('.get_passport_booking_id($sq_passport_info['passport_id'],$year).' )';
  global $model;
...
public function finance_save($refund_id)
{
	...
	$sq_passport = mysqli_fetch_assoc(mysqlQuery("select * from passport_master where passport_id='$passport_id'"));
