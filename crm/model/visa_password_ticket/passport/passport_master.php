...
public function passport_master_save(){
	...
	if(!$sq_passport){
		...
		for($i=0; $i<sizeof($first_name_arr); $i++){
			...
			$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(entry_id) as max from passport_master_entries"));
...
public function bank_cash_book_save($passport_id, $payment_id, $branch_admin_id,$particular){
	...
	$bank_id = $bank_id;
	$particular = get_sales_paid_particular(get_passport_booking_id($passport_id,$yr2), $payment_date, $payment_amount, $customer_id, $payment_mode, get_passport_booking_id($passport_id,$yr2),$bank_id,$transaction_id);
	$clearance_status = ($payment_mode=="Cheque" || $payment_mode=="Credit Card") ? "Pending" : "";
	$payment_side = "Debit";
...
public function finance_update($sq_passport_info, $row_spec,$particular){
	...
	$passport_sale_amount = $passport_issue_amount;
	$sq_pass = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as payment_amount from passport_payment_master where passport_id='$passport_id'"));
