...
public function passport_cancel_save()
{
	...
	for($i=0; $i<sizeof($entry_id_arr); $i++){
		$sq_cancel = mysqlQuery("update passport_master_entries set status='Cancel' where entry_id='$entry_id_arr[$i]'");
		if(!$sq_cancel){
...
public function cancel_mail_send($entry_id_arr)
{
	...
	$sq_entry = mysqli_fetch_assoc(mysqlQuery("select * from passport_master_entries where entry_id='$entry_id_arr[0]'"));
	$sq_passport_info = mysqli_fetch_assoc(mysqlQuery("select * from passport_master where passport_id='$sq_entry[passport_id]'"));
	...
	for($i=0; $i<sizeof($entry_id_arr); $i++)
	{
	$sq_entry = mysqli_fetch_assoc(mysqlQuery("select * from passport_master_entries where entry_id='$entry_id_arr[$i]'"));
