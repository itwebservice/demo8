...
public function ticket_payment_master_save()
{
	...
	if(!$sq_payment){
		...
		if($GLOBALS['flag']){
			...
			echo "Flight Ticket payment saved successfully!";
			exit;	
		}
		else{
			rollback_t();
...
public function finance_save($payment_id, $branch_admin_id)
{
	...
	if($payment_mode != 'Credit Note'){
		...
		if($payment_mode == 'Credit Card'){
			...
			$payment_particular = get_sales_paid_particular(get_ticket_booking_id($ticket_id,$yr1), $payment_date, $credit_charges, $customer_id, $payment_mode, get_ticket_booking_id($ticket_id,$yr1),$bank_id1,$transaction_id1);
			...
			$payment_date = $payment_date;
			$payment_particular = get_sales_paid_particular(get_ticket_booking_id($ticket_id,$yr1), $payment_date, $finance_charges, $customer_id, $payment_mode, get_ticket_booking_id($ticket_id,$yr1),$bank_id1,$transaction_id1);
			$ledger_particular = get_ledger_particular('By','Cash/Bank');
			$gl_id = 231;
			$payment_side = "Debit";
			$clearance_status = ($payment_mode=="Cheque"||$payment_mode=="Credit Card") ? "Pending" : "";
			...
			$payment_date = $payment_date;
			$payment_particular = get_sales_paid_particular(get_ticket_booking_id($ticket_id,$yr1), $payment_date, $credit_company_amount, $customer_id, $payment_mode, get_ticket_booking_id($ticket_id,$yr1),$bank_id1,$transaction_id1);
			$ledger_particular = get_ledger_particular('By','Cash/Bank');
			$gl_id = $company_gl;
			$payment_side = "Debit";
			$clearance_status = ($payment_mode=="Cheque"||$payment_mode=="Credit Card") ? "Pending" : "";
...
public function ticket_payment_master_update()
{
	...
	if(!$sq_payment){
		...
		if($GLOBALS['flag']){
			...
			echo "Flight Ticket payment update successfully!";
			exit;	
		}
		else{
			rollback_t();
...
public function payment_email_notification_send($ticket_id, $payment_amount, $payment_mode, $payment_date)
{
   ...
   global $model;
	$model->generic_payment_mail('48',$payment_amount, $payment_mode, $total_amount, $total_pay_amt, $payment_date, $due_date,$email_id,$subject,  $customer_name);
}
