...
public function vendor_save()
{
	...
	$vendor_count = mysqli_num_rows(mysqlQuery("select vendor_name from ticket_vendor where vendor_name='$vendor_name'"));
	if($vendor_count>0){
		rollback_t();
	...
	if($sq_vendor){
	    ...
	    if($GLOBALS['flag']){
	      commit_t();
	      echo "Flight Ticket Supplier has been successfully saved.";
	      exit;
	    }
	    else{
	      rollback_t();     
...
public function vendor_update()
{
	...
	$mobile_no = $encrypt_decrypt->fnEncrypt($mobile_no, $secret_key);
	$email_id = $encrypt_decrypt->fnEncrypt($email_id, $secret_key);
	$vendor_count = mysqli_num_rows(mysqlQuery("select vendor_name from ticket_vendor where vendor_name='$vendor_name' and vendor_id!='$vendor_id'"));
	if($vendor_count>0){
		rollback_t();
	...
	if($sq_vendor){
	    ...
	    if($GLOBALS['flag']){
	      commit_t();
	      echo "Flight Ticket Supplier has been successfully updated.";
	      exit;
	    }
	    else{
	      rollback_t();
