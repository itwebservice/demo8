...
class taxes_rules_master{
    function save(){
        ...
        $sq_max = mysqli_fetch_assoc(mysqlQuery("select max(rule_id) as max from tax_master_rules"));
        $rule_id = $sq_max['max'] + 1;
        $sq = mysqlQuery("INSERT INTO `tax_master_rules`(`rule_id`, `entry_id`, `name`, `validity`, `from_date`, `to_date`, `ledger_id`, `travel_type`, `calculation_mode`, `target_amount`, `conditions`,`applicableOn`,`status`,`created_at`) VALUES ('$rule_id','$entry_id','$name','$validity','$from_date','$to_date','$ledger','$travel_type','$calc_mode','$target_amount','$cond_arr','$applicableOn','Active','$created_at')");
    ...
    function clone_rule(){
        ...
        $result = mysqlQuery("SHOW COLUMNS FROM tax_master_rules"); 
        while ($r=mysqli_fetch_assoc($result)) {
            $cols[]= $r["Field"];
        }
   
       $result = mysqlQuery("SELECT * FROM tax_master_rules WHERE rule_id='$rule_id'");
