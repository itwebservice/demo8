...

{
	...
	for($i=0; $i<sizeof($entry_id_arr); $i++){
		...
		$sq_cancel = mysqlQuery("update excursion_master_entries set status='Cancel' where entry_id='$entry_id_arr[$i]'");
	...
	echo "Activity Booking has been successfully cancelled.";
...

{
	...
	$sq_entry = mysqli_fetch_assoc(mysqlQuery("select * from excursion_master_entries where entry_id='$entry_id_arr[0]'"));

	$sq_exc_info = mysqli_fetch_assoc(mysqlQuery("select * from excursion_master where exc_id='$sq_entry[exc_id]'"));
	...
	{
	  $sq_entry = mysqli_fetch_assoc(mysqlQuery("select * from excursion_master_entries where entry_id='$entry_id_arr[$i]'"));	
	  $sq_exc = mysqli_fetch_assoc(mysqlQuery("select * from excursion_master_tariff where entry_id = '$sq_entry[exc_name]'"));
	...
	$model->app_email_send('42',$cust_name,$email_id, $content,$subject);
