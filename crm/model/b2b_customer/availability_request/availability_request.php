...
class b2b_request{
    function availability_request(){
        ...
        if($sq_timestamp == 0){
            ...
            $sq_request = mysqlQuery("insert into hotel_availability_request (request_id,register_id,cust_name,concern_person,mobile_no,email_id,cart_data,status,created_at,unique_timestamp) values ('$request_id','$register_id','$sq_reg[company_name]','$guest_name','$sq_reg[mobile_no]','$sq_reg[email_id]','$hotel_list_arr','false','$date','$unique_timestamp')");

            if($sq_request){
                $this->request_mail_toAdmin($request_id,$sq_reg['company_name'],$hotel_list_arr);
                mysqlQuery("update b2b_registration set request_id='$request_id' where register_id='$register_id'");
