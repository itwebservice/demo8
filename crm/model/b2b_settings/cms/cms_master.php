...
public function save(){
	...
	if($section_name == '1'||$section_name == '2'||$section_name == '3'||$section_name == '4'||$section_name == '5'||$section_name == '6'||$section_name == '7'||$section_name == '8'||$section_name == '9'){
		...
		if($sq_settings == '0'){
			...
			$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(entry_id) as max from b2b_settings"));
			...
			$row_query = mysqli_fetch_assoc(mysqlQuery("SELECT * FROM `b2b_settings`"));
			if($section_name == '1'){
				$new_array = array();
	   ...
	   if($sq_settings == '0'){
			...
			if($activeTab == 'hotels'){
				...
				$sq_setting = mysqlQuery("insert into b2b_settings_second (terms_cond,privacy_policy,cancellation_policy,refund_policy,careers_policy,footer_strip)values('$entry_id1','$terms_cond','$privacy_policy','$cancellation_policy','$refund_policy','$careers_policy','$copyright_text')");
			}
			if($sq_setting){
				echo "sucess--B2B CMS settings saved.";
				exit;
			...
			if($activeTab == 'hotels'){
				...
				$sq_setting = mysqlQuery("update b2b_settings_second set terms_cond = '$terms_cond',privacy_policy = '$privacy_policy',cancellation_policy = '$cancellation_policy',refund_policy = '$refund_policy',careers_policy = '$careers_policy',footer_strip = '$copyright_text' where entry_id='1'");
			}
		}
...
public function delete(){
	...
	$banner_images = ($banner_images!='' && $banner_images != NULL) ? json_decode(json_encode($banner_images)) : [];
	if($section_name == '1'){
		$new_array = array();
		for($j=0;$j<sizeof($banner_images);$j++){
