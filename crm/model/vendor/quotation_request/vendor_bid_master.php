...
public function vendor_bid_save()
{
	...
	$sq_bid = mysqlQuery("update vendor_request_vendor_entries set bid_amount='$bid_amount', bid_status='Done',vendor_specification='$vendor_specification' where entry_id='$entry_id'");
	if($sq_bid){
		commit_t();
	...
	for($i=0;$i<count($service_arr);$i++)
	{
		if($entry_id_arr[$i]=="")
		{
			$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(response_id) as max from vendor_response_entries"));
				$response_id = $sq_max['max'] + 1;
			$sq_query=mysqlQuery("INSERT INTO vendor_response_entries(response_id, entry_id, services, description, amount) VALUES ('$response_id','$entry_id','$service_arr[$i]','$description_arr[$i]','$amount_arr[$i]')");			
			if(!$sq_query)
			{
				$GLOBALS['flag'] = false;
		...
		{
			$sq_update=mysqlQuery("UPDATE vendor_response_entries SET entry_id='$entry_id' ,services='$service_arr[$i]',description='$description_arr[$i]', amount='$amount_arr[$i]' WHERE response_id='$entry_id_arr[$i]'");
			if(!$sq_update)
			{
				$GLOBALS['flag'] = false;
...
function vendor_acknowlage($entry_id,$amount,$note,$vendor_name,$email)
{
	global $app_email_id, $app_name, $app_contact_no, $admin_logo_url, $app_website;
 	 ...
 	 $sq_request_id=mysqli_fetch_assoc(mysqlQuery("select * from vendor_request_vendor_entries where entry_id='$entry_id'"));
	 $content = '
	 <style type="text/css">
		td,th
		{
	    ...
	    <table style="border-collapse: collapse;">
	        ...
	        ';
	        $count=1;
	        $sq_quotation_record=mysqlQuery("select * from vendor_response_entries where entry_id='$entry_id'");
	        while($sq_row=mysqli_fetch_assoc($sq_quotation_record))
	        {
	        $content .='<tr>
	  ...
	  global $model;
	  $subject = " Vendor Quotation Acknowledgement for ".ge_vendor_request_id($sq_request_id['request_id']);
