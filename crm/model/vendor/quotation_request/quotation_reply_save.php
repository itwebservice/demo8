...
class quotation_reply_master_save{
	...
	public function quotation_reply_save()
	{
		...
		$currency_name = mysqli_fetch_assoc(mysqlQuery("select default_currency from currency_name_master where id=".$currency_code));
		begin_t();
		$sq_count = mysqli_num_rows(mysqlQuery("select * from vendor_reply_master where request_id='$request_id' and supplier_id='$supplier_id' and quotation_for='Transport Vendor' "));
		if($sq_count!=0){
			echo "Quotation reply already send!!!";
		}
		else{
			$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(id) as max from vendor_reply_master"));
			$id = $sq_max['max']+1;
			$enquiry_spec = addslashes($enquiry_spec);
			$sq_request = mysqlQuery("insert into vendor_reply_master(id,request_id, quotation_for, supplier_id, transport_cost, total_cost, currency_code, enquiry_spec , created_at,created_by,enquiry_id) values ('$id','$request_id', 'Transport Vendor', '$supplier_id', '$transport_cost', '$total_cost', '$currency_code', '$enquiry_spec' , '$created_at', '$created_by','$enquiry_id')");
	...
	public function hotel_quotation_reply_save()
	{
		...
		$enquiry_spec = addslashes($enquiry_spec);
		$currency_name = mysqli_fetch_assoc(mysqlQuery("select default_currency from currency_name_master where id=".$currency_code));
		$sq_count = mysqli_num_rows(mysqlQuery("select * from vendor_reply_master where request_id='$request_id' and supplier_id='$supplier_id'and quotation_for='Hotel Vendor' "));
		if($sq_count!=0){
			echo "Quotation reply already send!!!";
		}
		else{
...
{	
	...
	begin_t();
	$enquiry_spec = addslashes($enquiry_spec);
	$sq_count = mysqli_num_rows(mysqlQuery("select * from vendor_reply_master where request_id='$request_id' and supplier_id='$supplier_id'and quotation_for='DMC Vendor' "));
	$currency_name = mysqli_fetch_assoc(mysqlQuery("select default_currency from currency_name_master where id=".$currency_code));
	if($sq_count!=0){
		echo "Quotation reply already send!!!";
	}
...
public function quotation_reply_email($request_id,$hotel_cost,$total_cost,$currency_code,$enquiry_spec,$created_by,$created_at,$for,$transport_cost,$excursion_cost,$visa_cost,$enquiry_id){
    ...
    if($for=='Transport'){
		$sq_request=mysqli_fetch_assoc(mysqlQuery("select * from vendor_reply_master where request_id='$request_id' "));
		$sq_count = mysqli_fetch_assoc(mysqlQuery("select * from vendor_request_master where request_id='$sq_request[request_id]' "));
		$enq_customer_name = mysqli_fetch_assoc(mysqlQuery("select * from enquiry_master where enquiry_id = ".$sq_count['enquiry_id']));
		$date = $enq_customer_name['enquiry_date'];
		$yr = explode("-", $date);
		$year = $yr[0];
		$enquiry_id = get_enquiry_id($enq_customer_name['enquiry_id'],$year);
	    $supp_name = mysqli_fetch_assoc(mysqlQuery("select transport_agency_name as name from transport_agency_master where transport_agency_id = '$sq_request[supplier_id]'"));
		$content = '
			<tr>
	...
	if($for=='Hotel'){
	   ...
	   $enq_customer_name = mysqli_fetch_assoc(mysqlQuery("select * from enquiry_master where enquiry_id = ".$sq_count['enquiry_id']));
		...
		$year = $yr[0];
		$enquiry_id = get_enquiry_id($enq_customer_name['enquiry_id'],$year);
		$supp_name = mysqli_fetch_assoc(mysqlQuery("select hotel_name as name from hotel_master where hotel_id= '$sq_request[supplier_id]'"));
		$content = '
		<tr>
			<table width="85%" cellspacing="0" cellpadding="5" style="color: #888888;border: 1px solid #888888;margin: 0px auto;margin-top:20px; min-width: 100%;" role="presentation">
			<tr><td style="text-align:left;border: 1px solid #888888;">Request ID</td>   <td style="text-align:left;border: 1px solid #888888;">'.ge_vendor_request_id($request_id,$year).'</td></tr>
	...
	if($for=='DMC'){
		...
		$year = $yr[0];
		$enquiry_id = get_enquiry_id($enq_customer_name['enquiry_id'],$year);
		$supp_name = mysqli_fetch_assoc(mysqlQuery("select company_name as name from dmc_master where dmc_id = '$sq_request[supplier_id]'"));
	...
	$subject = "Supplier Quotation Reply ($supp_name[name], $enq_customer_name[name],$enquiry_id)";
	global $model;
	$model->app_email_send('24',"Admin",$app_email_id, $content,'');
}
