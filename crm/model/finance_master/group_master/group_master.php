...
public function subgroup_master_save()
{
	...
	$sq_count = mysqli_num_rows(mysqlQuery("select subgroup_name from subgroup_master where subgroup_name='$subgroup_name'"));
	if($sq_count>0){
		echo "error--".$subgroup_name." already exists!";
		exit;
	}

	$sq_max = mysqli_fetch_assoc(mysqlQuery("select max(subgroup_id) as max from subgroup_master"));
...
public function subgroup_master_update()
{
	...
	$sq_count = mysqli_num_rows(mysqlQuery("select subgroup_name from subgroup_master where subgroup_name='$subgroup_name' and subgroup_id!='$subgroup_id'"));
	if($sq_count>0){
		echo "error--".$subgroup_name." already exists!";
		exit;
	}
	...
	begin_t();
	$subgroup_name = addslashes($subgroup_name);
	$sq_bank = mysqlQuery("update subgroup_master set subgroup_name='$subgroup_name', group_id='$group_id' where subgroup_id='$subgroup_id'");
	if($sq_bank){
		commit_t();
