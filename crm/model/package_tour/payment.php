...
///////////////////////////////////***** Package Tour payment master save start *********/////////////////////////////////////////////////////////////
 ...
 public function payment_finance_save($payment_id)
 {
    ...
    if($payment_mode != 'Credit Note'){
      ...
      $payment_particular = get_sales_paid_particular(get_package_booking_payment_id($booking_id,$yr1), $payment_date, $credit_charges, $customer_id, $payment_mode, get_package_booking_id($booking_id,$yr1),$bank_id1,$transaction_id1);
      ...
      $payment_amount = $credit_charges;
      $payment_date = $payment_date;
      $payment_particular = get_sales_paid_particular(get_package_booking_payment_id($booking_id,$yr1), $payment_date1, $credit_charges, $customer_id, $payment_mode, get_package_booking_id($booking_id,$yr1),$bank_id1,$transaction_id1);
      $ledger_particular = get_ledger_particular('By','Cash/Bank');
      $gl_id = 224;
      $payment_side = "Credit";
      $clearance_status = ($payment_mode=="Cheque"||$payment_mode=="Credit Card") ? "Pending" : "";
...
// $finance_charges = number_format($finance_charges,2);
      ...
      $payment_date = $payment_date;
      $payment_particular = get_sales_paid_particular(get_package_booking_payment_id($booking_id,$yr1), $payment_date1, $finance_charges, $customer_id, $payment_mode, get_package_booking_id($booking_id,$yr1),$bank_id1,$transaction_id1);
      $ledger_particular = get_ledger_particular('By','Cash/Bank');
      $gl_id = 231;
      $payment_side = "Debit";
      $clearance_status = ($payment_mode=="Cheque"||$payment_mode=="Credit Card") ? "Pending" : "";
      ...
      $payment_date = $payment_date;
      $payment_particular = get_sales_paid_particular(get_package_booking_payment_id($booking_id,$yr1), $payment_date, $credit_company_amount, $customer_id, $payment_mode, get_package_booking_id($booking_id,$yr1),$bank_id1,$transaction_id1);
      $ledger_particular = get_ledger_particular('By','Cash/Bank');
      $gl_id = $company_gl;
      $payment_side = "Debit";
      $clearance_status = ($payment_mode=="Cheque"||$payment_mode=="Credit Card") ? "Pending" : "";
 ...
 public function bank_cash_book_save($payment_id)
{
  ...
  $sq_booking = mysqli_fetch_assoc(mysqlQuery("select customer_id from tourwise_traveler_details where id='$sq_booking_info[tourwise_traveler_id]'"));
  $credit_charges = $_POST['credit_charges'];
	$credit_card_details = $_POST['credit_card_details'];
...
function finance_update($sq_payment_info, $clearance_status1)
{
    ...
    if($payment_amount1 != $payment_old_value){
      ...
      if($payment_mode != 'Credit Note'){
        ...
        $payment_date = $payment_date;
        $payment_particular = get_sales_paid_particular(get_package_booking_payment_id($payment_id,$yr2), $payment_date, $credit_charges_old, $customer_id, $payment_mode, get_package_booking_id($booking_id,$yr2),$bank_id,$transaction_id1);
        $ledger_particular = get_ledger_particular('By','Cash/Bank');
        $gl_id = $cust_gl;
        $payment_side = "Credit";
        $clearance_status = ($payment_mode=="Cheque"||$payment_mode=="Credit Card") ? "Pending" : "";
        ...
        $payment_amount = $finance_charges;
        $payment_date = $payment_date;
        $payment_particular = get_sales_paid_particular(get_package_booking_payment_id($payment_id,$yr2), $payment_date, $finance_charges, $customer_id, $payment_mode, get_package_booking_id($booking_id,$yr2),$bank_id,$transaction_id1);
        $ledger_particular = get_ledger_particular('By','Cash/Bank');
        $gl_id = 231;
        $payment_side = "Credit";
        $clearance_status = ($payment_mode=="Cheque"||$payment_mode=="Credit Card") ? "Pending" : "";
        ...
        $payment_amount = $credit_company_amount;
        $payment_date = $payment_date;
        $payment_particular = get_sales_paid_particular(get_package_booking_payment_id($payment_id,$yr2), $payment_date, $credit_company_amount, $customer_id, $payment_mode, get_package_booking_id($booking_id,$yr2),$bank_id,$transaction_id1);
        $ledger_particular = get_ledger_particular('By','Cash/Bank');
        $gl_id = $company_gl;
        $payment_side = "Credit";
        $clearance_status = ($payment_mode=="Cheque"||$payment_mode=="Credit Card") ? "Pending" : "";
        ...
        $payment_date = $payment_date;
        $payment_particular = get_sales_paid_particular(get_package_booking_payment_id($payment_id,$yr2), $payment_date, $payment_old_value, $sq_group_info['customer_id'], $payment_mode, get_package_booking_id($booking_id,$yr2),$bank_id,$transaction_id1);
        $ledger_particular = get_ledger_particular('By','Cash/Bank');
        $gl_id = $pay_gl;
        $payment_side = "Credit";
        $clearance_status = ($payment_mode=="Cheque" || $payment_mode == 'Credit Card') ? "Pending" : "";
...
public function bank_cash_book_update($sq_payment_info, $clearance_status)
{
  ...
  $transaction_id = $transaction_id;
  $bank_id = $bank_id;
  $particular = get_sales_paid_particular(get_package_booking_payment_id($payment_id,$yr1), $payment_date, $payment_amount, $sq_booking_info['customer_id'], $payment_mode, get_package_booking_id($sq_payment_info['booking_id'],$yr1),$bank_id,$transaction_id);
...
public function payment_email_notification_send($booking_id, $payment_amount, $payment_mode, $payment_date){
  ...
  $customer_name = ($sq_customer_info['type'] == 'Corporate' || $sq_customer_info['type'] == 'B2B') ? $sq_customer_info['company_name'] : $sq_customer_info['first_name'].' '.$sq_customer_info['last_name'];
   ...
   global $model;
   $model->generic_payment_mail('45',$payment_amount, $payment_mode, $total_amount, $paid_amount, $payment_date, $due_date,$sq_booking['email_id'], $subject,$customer_name,$sq_booking['currency_code']);
 }
