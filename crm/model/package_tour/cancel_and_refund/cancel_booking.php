...
public function cancel_booking_save($traveler_id_arr, $booking_id)
{
	for($i=0; $i<sizeof($traveler_id_arr); $i++){
		...
		$sq = mysqlQuery("update package_travelers_details set status='Cancel' where traveler_id='$traveler_id_arr[$i]'");
		if(!$sq){
...
public function traveler_cancelation_mail_send($traveler_id_arr, $booking_id)
{
  ...
  $sq_tour = mysqli_fetch_assoc(mysqlQuery("select * from package_tour_booking_master where booking_id='$booking_id'"));
  ...
  $yr = explode("-", $date);
  $year =$yr[0];
  $sq_personal_info1 = mysqlQuery("select * from package_travelers_details where booking_id='$booking_id'");
  $content1 = '';

    $count = 0;
    while($sq_personal_info = mysqli_fetch_assoc($sq_personal_info1)){
  ...
  $model->app_email_send('28',$cust_name,$email_id, $content,$subject);
}
