...
{ 
    ...
    if(!$sq_booking)
    {
      ...
      $this->package_tour_tranpsort_information_save($max_booking_id, $transp_vehicle_arr, $transp_start_date,$trans_pickuptype_arr,$trans_pickup_arr,$trans_droptype_arr,$trans_drop_arr,$trans_count_arr,$transp_end_date);
    ...
    if($GLOBALS['flag']){
      ...
      $payment_amount1 = $payment_amount[0];
      if($payment_mode[0] == 'Credit Note'){
...
//Transport Save
function package_tour_tranpsort_information_save($max_booking_id, $transp_vehicle_arr, $transp_start_date,$trans_pickuptype_arr,$trans_pickup_arr,$trans_droptype_arr,$trans_drop_arr,$trans_count_arr,$transp_end_date){
    for($i=0; $i<sizeof($transp_vehicle_arr); $i++){
        ...
        $sq = mysqlQuery("INSERT INTO `package_tour_transport_master`(`entry_id`, `booking_id`, `transport_bus_id`, `transport_from_date`,`transport_end_date`, `pickup`, `pickup_type`, `drop`, `drop_type`, `vehicle_count`) values ('$max_entry_id', '$max_booking_id', '$transp_vehicle_arr[$i]', '$transp_start_date[$i]','$transp_end_date[$i]', '$pickup', '$pickup_type', '$drop', '$drop_type', '$trans_count_arr[$i]')");
