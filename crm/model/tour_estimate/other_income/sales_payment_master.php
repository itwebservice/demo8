	...
	for($i=0;$i<sizeof($purchase_type_arr);$i++){
		...
		if($purchase_type_arr[$i]=='Group Booking')
		{
			...
			while($row_booking = mysqli_fetch_assoc($sq_booking)){
				...
				//Cancel traveler
				$cancel_est_tr=mysqli_fetch_assoc(mysqlQuery("select * from refund_traveler_estimate where tourwise_traveler_id='$row_booking[id]'"));
				$can_travel_tr = $cancel_est_tr['cancel_amount'];
				$can_tour_tr = $cancel_est_tr['total_tour_amount'];
				
				//Cancel tour
				$cancel_est_tour=mysqli_fetch_assoc(mysqlQuery("select * from refund_tour_estimate where tourwise_traveler_id='$row_booking[id]'"));
				$can_travel_tour = $cancel_est_tour['cancel_amount'] ;
				$can_tour_tour = $cancel_est_tour['total_tour_amount'];

				//Paid
				$sq_tr_pay=mysqli_fetch_assoc(mysqlQuery("select sum(amount) as travel_sum from payment_master where tourwise_traveler_id='$row_booking[id]' and payment_for = 'Travelling' and clearance_status!='Pending' AND clearance_status!='Cancelled'"));
				$total_tr_pay = $sq_tr_pay['travel_sum'];
				$sq_tour_pay=mysqli_fetch_assoc(mysqlQuery("select sum(amount) as tour_sum from payment_master where tourwise_traveler_id='$row_booking[id]' and payment_for = 'Tour' and clearance_status!='Pending' AND clearance_status!='Cancelled'"));
		...
		if($purchase_type_arr[$i]=='Package Booking')
		{
			...
			while($row_booking = mysqli_fetch_assoc($sq_booking)){
				...
				//Paid
				$sq_tr_pay=mysqli_fetch_assoc(mysqlQuery("select sum(amount) as travel_sum from package_payment_master where booking_id='$row_booking[booking_id]' and payment_for = 'Travelling' and clearance_status!='Pending' AND clearance_status!='Cancelled'"));
				$total_tr_pay = $sq_tr_pay['travel_sum'];
				$sq_tour_pay=mysqli_fetch_assoc(mysqlQuery("select sum(amount) as tour_sum from package_payment_master where booking_id='$row_booking[booking_id]' and payment_for = 'Tour' and clearance_status!='Pending' AND clearance_status!='Cancelled'"));
				$total_tour_pay = $sq_tour_pay['tour_sum'];
				$pay_amount = 0;
...
public function finance_save($payment_id,$invoice_no,$invoice_name,$module_entry_id,$row_spec)
{
	...
	$sq_payment = mysqli_fetch_assoc(mysqlQuery("select max(finance_transaction_id) as payment_id from finance_transaction_master where module_name='Sale Receipt'"));
