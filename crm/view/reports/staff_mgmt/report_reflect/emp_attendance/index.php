...
$branch_status_att = $sq1['branch_status'];
?>
	...
	<div class="row">
		...
		<div class="col-md-3 col-sm-6 mg_bt_10_xs">
			<select name="year_filter" style="width: 100%" id="year_filter" title="Year">
				...
				<?php 
				for($year_count=2021; $year_count<2099; $year_count++){
					?>
		...
		<div class="col-md-3 mg_bt_10_xs">
			<select id="emp_id_filter" name="emp_id_filter" title="User Name" style="width: 100%">
				...
				if($branch_status_att=='yes' && $role!='Admin'){
		...
		<input type="hidden" id="branch_status_att" value="<?= $branch_status_att ?>"/>
...
<div class="row mg_tp_20"> <div class="col-md-12 no-pad"> <div class="table-responsive">
	<table id="att_report" class="table table-hover mg_tp_20" style="margin: 20px 0 !important;">         
	</table>
</div></div></div>
</div>
...
function report_reflect(){
	...
	var branch_status_att = $('#branch_status_att').val();
...
function excel_report()
{
	...
	var days = parseInt(daysInMonth(month,year));
	if(year=='' || month==''){
		error_msg_alert("Year and Month filters are mandatory!");
		return false;
	}
	window.location = base_url+'view/reports/staff_mgmt/report_reflect/emp_attendance/excel_report.php?year='+year+'&month='+month+''+'&emp_id='+emp_id+'&branch_status_att='+branch_status_att+'&mdays='+days;
}
