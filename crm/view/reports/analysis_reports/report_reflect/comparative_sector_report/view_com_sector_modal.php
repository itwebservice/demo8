...
$qry = "SELECT airline_master.airline_id,count(*)as sector,sum(ticket_master.ticket_total_cost) as purchase,ticket_trip_entries.ticket_id,ticket_master.created_at,ticket_master.ticket_total_cost,customer_master.first_name,customer_master.last_name FROM airline_master INNER JOIN ticket_trip_entries on airline_master.airline_code = ticket_trip_entries.flight_no inner join ticket_master on ticket_trip_entries.ticket_id = ticket_master.ticket_id INNER JOIN customer_master on ticket_master.customer_id = customer_master.customer_id where ticket_trip_entries.departure_city='".$from_location."' AND ticket_trip_entries.arrival_city='".$to_location."' ".$_SESSION['dateqry'] ." Group by ticket_master.ticket_id ";
$sq_query = mysqlQuery($qry);

$qry1 = "SELECT plane_master.plane_id,count(*)as sector ,sum(tourwise_traveler_details.plane_expense) as purchase, plane_master.plane_id,customer_master.first_name,customer_master.last_name, tourwise_traveler_details.form_date FROM plane_master INNER JOIN tourwise_traveler_details on plane_master.tourwise_traveler_id = tourwise_traveler_details.traveler_group_id INNER JOIN airline_master on plane_master.company = airline_master.airline_id INNER JOIN customer_master on tourwise_traveler_details.customer_id = customer_master.customer_id INNER JOIN tour_master on tourwise_traveler_details.tour_id= tour_master.tour_id  where plane_master.from_location='".$from_location."'  AND plane_master.to_location='".$to_location."' ".$_SESSION['group_qry']." Group by plane_master.plane_id ";
$sq_query1 = mysqlQuery($qry1);

$qry2 = "SELECT package_plane_master.plane_id,count(*)as sector ,sum(package_plane_master.amount) as purchase, package_plane_master.plane_id,customer_master.first_name,customer_master.last_name, package_tour_booking_master.tour_from_date FROM package_plane_master INNER JOIN airline_master ON package_plane_master.company = airline_master.airline_id INNER JOIN package_tour_booking_master on package_plane_master.booking_id = package_tour_booking_master.booking_id INNER JOIN customer_master on package_tour_booking_master.customer_id = customer_master.customer_id where package_plane_master.from_location='".$from_location."'  AND package_plane_master.to_location='".$to_location."' ".$_SESSION['package_qry']."  Group by package_plane_master.plane_id";
