...
<?php
	...
	$count=0;
	$total_amount=0;
	$sq1 = mysqlQuery($query1);
	while($row1= mysqli_fetch_assoc($sq1))
	{				

		$sq = mysqli_fetch_assoc(mysqlQuery(" select first_name, last_name from emp_master where emp_id='$row1[booker_id]' "));
		$booker_name = $sq['first_name']." ".$sq['last_name'];

		$sq = mysqli_fetch_assoc(mysqlQuery("select tour_name from tour_master where tour_id='$row1[tour_id]'"));
		$tour_name = $sq['tour_name'];


		$sq = mysqli_fetch_assoc(mysqlQuery("select from_date, to_date from tour_groups where tour_id='$row1[tour_id]'"));
