...
<table class="table trable-hover" id="tour_completesummary" style=" width:100% ;margin: 20px 0 !important;">
    ...
    $sq_tourwise_details = mysqlQuery($query1);
	while($row_tourwise_details=mysqli_fetch_assoc($sq_tourwise_details))
	{
		$first_time = true;
		$sq_booker_info = mysqli_fetch_assoc(mysqlQuery("select * from emp_master where emp_id='$row_tourwise_details[emp_id]'"));
		if($sq_booker_info['first_name'] != ''){ $booker_name = $sq_booker_info['first_name']." ".$sq_booker_info['last_name'];
		}
		else{  $booker_name = 'Admin';}
		$sq_total_member_count = mysqli_num_rows(mysqlQuery("select traveler_id from travelers_details where traveler_group_id='$row_tourwise_details[traveler_group_id]'"));

		$sq_total_tour_paid_amount = mysqli_fetch_assoc(mysqlQuery("select sum(amount) as total_tour_paid from payment_master where tourwise_traveler_id='$row_tourwise_details[id]' and payment_for='Tour' and clearance_status!='Pending' and clearance_status!='Cancelled'"));

		$sq_total_travel_paid_amount = mysqli_fetch_assoc(mysqlQuery("select sum(amount) as total_travel_paid from payment_master where tourwise_traveler_id='$row_tourwise_details[id]' and payment_for='Travelling' and clearance_status!='Pending' and clearance_status!='Cancelled'"));

		$sq_branch_name = mysqli_fetch_assoc(mysqlQuery("SELECT * from branches where branch_id='$sq_booker_info[branch_id]'"));

		$sq_traveler_det = mysqlQuery("select * from travelers_details where traveler_group_id='$row_tourwise_details[traveler_group_id]'");
