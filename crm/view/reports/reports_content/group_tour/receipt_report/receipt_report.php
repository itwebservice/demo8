...
$sq_payment = mysqlQuery($payment_query);

while($row_payment = mysqli_fetch_assoc($sq_payment))
{
  $count++;

  $sq_receipt = mysqli_fetch_assoc(mysqlQuery("select * from receipt_master where payment_id='$row_payment[payment_id]' "));
  $receipt_id = $_SESSION['unique_receipt_id'].''.$sq_receipt['receipt_id'];

  $sq_tourwise_det = mysqli_fetch_assoc(mysqlQuery("select tour_id, tour_group_id, traveler_group_id from tourwise_traveler_details where id='$row_payment[tourwise_traveler_id]'"));

  $sq_traveler_det = mysqli_fetch_assoc(mysqlQuery("select first_name, last_name from travelers_details where traveler_group_id='$sq_tourwise_det[traveler_group_id]'"));

  $tour_name1 = mysqli_fetch_assoc(mysqlQuery("select tour_name from tour_master where tour_id= '$sq_tourwise_det[tour_id]'"));
  $tour_name = $tour_name1['tour_name'];

  $tour_group1 = mysqli_fetch_assoc(mysqlQuery("select from_date, to_date from tour_groups where group_id= '$sq_tourwise_det[tour_group_id]'"));
...
<div class="row">
  <div class="col-md-4">
    <select name="bank_name_reciept" id="bank_name_reciept" title="Bank Name" class="form-control">
      <?php 
      $sq_bank = mysqlQuery("select * from bank_name_master");
