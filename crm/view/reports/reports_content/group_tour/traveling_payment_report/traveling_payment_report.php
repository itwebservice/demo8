...
while($row1 = mysqli_fetch_assoc($sq1))
{
	...
	$sq_cancel_amt = mysqli_fetch_assoc(mysqlQuery("select * from refund_traveler_estimate where tourwise_traveler_id='$row1[id]'"));
	...
	$sq_pay_train=mysqli_fetch_assoc(mysqlQuery("SELECT sum(amount) as sum from payment_master where tourwise_traveler_id = '$row1[id]' and travel_type='train' and clearance_status!='Pending' AND clearance_status!='Cancelled'"));
	$sq_pay_air=mysqli_fetch_assoc(mysqlQuery("SELECT sum(amount) as sum from payment_master where tourwise_traveler_id = '$row1[id]' and travel_type='air' and clearance_status!='Pending' AND clearance_status!='Cancelled'"));
	$sq_pay_both=mysqli_fetch_assoc(mysqlQuery("SELECT sum(amount) as sum from payment_master where tourwise_traveler_id = '$row1[id]' and travel_type='both' and clearance_status!='Pending' AND clearance_status!='Cancelled'"));
