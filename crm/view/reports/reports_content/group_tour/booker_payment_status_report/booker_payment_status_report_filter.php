...
while($row1 = mysqli_fetch_assoc($sq1))
{
	...
	$sq2 = mysqlQuery("select * from payment_master where tourwise_traveler_id='$row1[id]' AND clearance_status!='Pending' AND clearance_status!='Cancelled'");
	...
	$sq3 = mysqli_fetch_assoc(mysqlQuery("select tour_name from tour_master where tour_id='$row1[tour_id]'"));
	$tour_name = $sq3['tour_name'];

	$sq3 = mysqli_fetch_assoc(mysqlQuery("select from_date, to_date from tour_groups where group_id='$row1[tour_group_id]'"));
