...
{
	$first_time = true;
	$sq_booker_info=mysqli_fetch_assoc(mysqlQuery("SELECT * from emp_master where emp_id='$row_tourwise_details[booker_id]'"));
	...
	$sq_total_travel_paid_amount = mysqli_fetch_assoc(mysqlQuery("select sum(amount) as total_travel_paid from payment_master where tourwise_traveler_id='$row_tourwise_details[id]' and payment_for='traveling' and clearance_status!='Pending' AND clearance_status!='Cancelled'"));

	$sq_traveler_det = mysqlQuery("select * from travelers_details where traveler_group_id='$row_tourwise_details[traveler_group_id]'");

	$sq_branch=mysqli_fetch_assoc(mysqlQuery("select * from branches where branch_id='$sq_booker_info[branch_id]'"));
