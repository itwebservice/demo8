...
<?php
  ...
  $sq = mysqlQuery("select id, traveler_group_id from tourwise_traveler_details where tour_id = '$tour_id' and tour_group_id = '$tour_group_id' and tour_group_status!='Cancel' ");
  ...
  {
  	$sq = mysqlQuery("select * from travelers_details where traveler_group_id = '$traveler_group_id_arr[$i]' and status='Active'");
  	while($row = mysqli_fetch_assoc($sq))
  	{
