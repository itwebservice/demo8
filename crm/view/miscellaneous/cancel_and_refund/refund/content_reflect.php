		...
		<table class="table table-hover table-bordered" id="tbl_refund_list" style="margin: 0 !important;">
			...
			<tbody>
				...
				while($row_visa_refund = mysqli_fetch_assoc($sq_visa_refund)){
					...
					$sq_visa_info = mysqli_fetch_assoc(mysqlQuery("select * from miscellaneous_master where misc_id='$row_visa_refund[misc_id]'"));
			...
			<tfoot>
				...
				<tr class="active">
					<th colspan="2" class="info">Refund : <?= number_format($sq_pay['sum'],2) ?></th>
					...
					<th colspan="2" class="success">Total_Refund : <?= number_format(($sq_pay['sum'] - $sq_pend_pay['sum'] - $sq_canl_pay['sum'] ), 2); ?></th>
				</tr>
			</tfoot>
		</table>
		</div> 
