				...
				<table class="table table-bordered no-marg">
					...
					<tbody>
						...
						if($sq_req['quotation_for']=="Transport"){
							$sq_transport = mysqlQuery("select * from transport_agency_master where transport_agency_id in ($sq_req[city_id])");
							while($row_trans = mysqli_fetch_assoc($sq_transport)){
						...
						if($sq_req['quotation_for']=="DMC"){
							$sq_dmc = mysqlQuery("select * from dmc_master where dmc_id in ($sq_req[city_id])");
						...
						if($sq_req['quotation_for']=="Hotel"){
							$sq_hotel = mysqlQuery("select * from hotel_master where hotel_id in ($sq_req[city_id])");
