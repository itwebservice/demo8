...
$sq_bus_det = mysqli_fetch_assoc(mysqlQuery("select * from bus_master where bus_id='$bus_id'"));
?>
<div class="col-md-6">
	...
	$sq_traveler_det = mysqlQuery("select * from travelers_details where status!='Cancel' and traveler_group_id in (select traveler_group_id from tourwise_traveler_details where tour_id='$tour_id' and tour_group_id='$tour_group_id')");
	while($row_traveler_det = mysqli_fetch_assoc($sq_traveler_det))
	{
		$sq_seat_booking_det = mysqli_fetch_assoc(mysqlQuery("select seat_no from bus_seat_arrangment_master where tour_id='$tour_id' and tour_group_id='$tour_group_id' and bus_id='$bus_id' and traveler_id='$row_traveler_det[traveler_id]' "));
