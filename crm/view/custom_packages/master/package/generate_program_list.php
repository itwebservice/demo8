...
<table id="dynamic_table_list" style="width:100%" name="dynamic_table_list">
	...
	for($i = 1; $i<=$total_days; $i++)	{ 
		...
		<tr>
			...
			<td class='col-md-1 pad_8'><button type="button" class="btn btn-excel btn-sm" title="Add Itinerary" id="itinerary<?php echo $i; ?>" onclick="add_itinerary('dest_name_s','special_attaraction<?php echo $i; ?>','day_program<?php echo $i; ?>','overnight_stay<?php echo $i; ?>','Day-<?=$i?>')"><i class="fa fa-plus"></i></button>
			</td>
		</tr>
	<?php } ?>
</table>
