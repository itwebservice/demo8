...
$type="rule_master";
$sq_rules = mysqli_fetch_assoc(mysqlQuery("select * from tax_master_rules where rule_id='$rule_id'"));

$conditions = json_decode($sq_rules['conditions']);
$sq_ledger =  mysqli_fetch_assoc(mysqlQuery("select ledger_id,ledger_name from ledger_master where ledger_id='$sq_rules[ledger_id]'"));
      ...
      <div class="modal-body">
        <form id="frm_tax_rules_update">
          ...
          <div class="row"> 
              ...
              <div class="col-md-3">
                  <select name="target_amount" id="target_amount" data-toggle="tooltip" class="form-control" onchange="discount_on_purchase()" title="Target Amount" required>
          ...
          <div class="row mg_tp_10">
          <div class="col-md-4">
              <input type="checkbox" id="applicable" name="applicable" autocomplete="off" <?= $checked ?> data-original-title="" title="" aria-hidden="true" class="labelauty" style="display: none;" onclick="inclusive_checker();discount_on_purchase();">
                ...
                <tr>
                    ...
                    <td style="width:25%"><select name="valueu<?= $i ?>" id="valueu<?= $i ?>" data-toggle="tooltip" class="form-control app_select2" title="Value" style="width:100%" <?= $status ?>>
                      <?php if($id == '1'){
                        ...
                        $sq_state = mysqli_fetch_assoc(mysqlQuery("select * from state_master where id='$value'")); ?>
