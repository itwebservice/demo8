		...
		<div class="table-responsive">
			<table class="table table-bordered" id="tbl_refund_list" style="margin: 0 !important;">
				...
				<tbody>
					...
					$sq_hotel_refund = mysqlQuery($query);
					while($row_hotel_refund = mysqli_fetch_assoc($sq_hotel_refund)){

						$count++;
						$total_refund = $total_refund+$row_hotel_refund['refund_amount'];

						$sq_hotel_info1 = mysqli_fetch_assoc(mysqlQuery("select * from hotel_booking_master where booking_id='$row_hotel_refund[booking_id]'"));
						...
						else if($row_hotel_refund['clearance_status']=='Cleared'){ $bg = "success"; }
						else{ $bg = ""; }
				...
				<tfoot>
					...
					<tr class="active">
						...
						<th class="text-right success" colspan="2">Total_Refund : <?= $sq_pay['sum'] - $sq_ref_pen_total['sum'] - $sq_ref_can_total['sum'] ?></th>
					</tr>
				</tfoot>
			</table>
			</div>
...
$(function(){
  $('#frm_refund_save').validate({
      ...
      submitHandler:function(form){
			...
			if(parseFloat(remaining_fund) == 0 && parseFloat(refund_amount) > 0){
