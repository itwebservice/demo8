...
$count = 0;
	while($row_req = mysqli_fetch_assoc($sq_query_login)){
		$sq_hotel = mysqli_fetch_assoc(mysqlQuery("select * from hotel_master where hotel_id='$row_req[hotel_id]'"));
		$sq_city = mysqli_fetch_assoc(mysqlQuery("select * from city_master where city_id='$row_req[city_id]'"));
