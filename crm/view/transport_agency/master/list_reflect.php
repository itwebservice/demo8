...
<table class="table table-bordered" id="tbl_transport_agency_list" style="margin: 20px 0 !important;">
	...
	<tbody>
		<?php 
		$count = 0;
		$sq_transport_agency = mysqlQuery($query);
		while($row_transport_agency = mysqli_fetch_assoc($sq_transport_agency)){
			$sq_city = mysqli_fetch_assoc(mysqlQuery("select * from city_master where city_id='$row_transport_agency[city_id]'"));
