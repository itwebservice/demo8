...
<table class="table table-bordered" id="tbl_vendor_list" style="margin: 20px 0 !important;">
	...
	<tbody>
		...
		while($row_vendor = mysqli_fetch_assoc($sq_vendor)){
			$sq_gl = mysqli_fetch_assoc(mysqlQuery("select * from gl_master where gl_id='$row_vendor[gl_id]'"));
			$sq_city = mysqli_fetch_assoc(mysqlQuery("select city_name from city_master where city_id='$row_vendor[city_id]'"));
