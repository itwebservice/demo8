			...
			<div class="row mg_bt_10">				
				<div class="col-xs-12">
				  	<select name="entry_id" id="entry_id" multiple>
				  		<?php 
				  		$sq_exc_entries = mysqli_fetch_assoc(mysqlQuery("select * from excursion_master where exc_id='$exc_id'"));
	...
	<div class="col-md-7 col-sm-12 col-xs-12">
		...
		<table class="table table-hover table-bordered" id="tbl_refund_list" style="margin: 0 !important;">
			...
			<tbody>
				...
				$count = 0;
				$bg;
				$sq_exc_refund = mysqlQuery($query);				
				while($row_exc_refund = mysqli_fetch_assoc($sq_exc_refund)){

					$count++;

					$sq_exc_info = mysqli_fetch_assoc(mysqlQuery("select * from excursion_master where exc_id='$row_exc_refund[exc_id]'"));

					$cust_name = "";
					$sq_refund_entries = mysqlQuery("select * from exc_refund_entries where refund_id='$row_exc_refund[refund_id]'");
					...
					while($row_refund_entry = mysqli_fetch_assoc($sq_refund_entries)){
						$sq_cust = mysqli_fetch_assoc(mysqlQuery("select * from excursion_master where customer_id='$row_refund_entry[entry_id]'"));
						$date = $sq_cust['created_at'];
						$yr = explode("-", $date);
						$year =$yr[0];
						$s_query = "select * from customer_master where customer_id='$sq_cust[customer_id]'";
						$sq_entry_info = mysqli_fetch_assoc(mysqlQuery($s_query));
						...
						$v_service_name = "Activity Booking";
			...
			<tfoot>
				...
				<tr class="active">
					...
					<th colspan="2" class="success">Total_Refund : <?= number_format($refund_amount1-$pend_amount-$can_amount,2); ?></th>
				</tr>
			</tfoot>
		</table>
		</div> 
...
$(function(){
  $('#frm_refund_save').validate({
      ...
      submitHandler:function(form){
			...
			if(Number(refund_amount) > Number(remaining_pay))
			{ error_msg_alert("Amount can not be greater than total refund amount"); return false; }
			else if(parseFloat(remaining_pay) == 0 && parseFloat(refund_amount) > 0){
