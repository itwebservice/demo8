		...
		while($row_exc_payment = mysqli_fetch_assoc($sq_exc_payment)){
			...
			$bg='';
				...
				if($row_exc_payment['payment_mode'] == 'Credit Note' || ($row_exc_payment['payment_mode'] == 'Credit Card' && $row_exc_payment['clearance_status']=="Cleared")){
					...
					$edit_btn = "<button class='btn btn-info btn-sm' data-toggle='tooltip' onclick='exc_payment_update_modal(".$row_exc_payment['payment_id'].")' title='Update Details'><i class='fa fa-pencil-square-o'></i></button>";
				}
				...
				$temp_arr = array( "data" => array(
					...
					'<a onclick="loadOtherPage(\''. $url1 .'\')" class="btn btn-info btn-sm" title="Download Receipt"><i class="fa fa-print"></i></a>'.$edit_btn
				), "bg" =>$bg );
