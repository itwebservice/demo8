...
$sq_ledger = mysqlQuery("select * from ledger_master where 1");
while($row_ledger = mysqli_fetch_assoc($sq_ledger)){

	$sq_sl = mysqli_fetch_assoc(mysqlQuery("select * from subgroup_master where subgroup_id='$row_ledger[group_sub_id]'"));
    $credit_balance = ($row_ledger['balance_side'] == 'Credit') ? $row_ledger['balance'] : 0;
    $debit_balance = ($row_ledger['balance_side'] == 'Debit') ? $row_ledger['balance'] : 0;
