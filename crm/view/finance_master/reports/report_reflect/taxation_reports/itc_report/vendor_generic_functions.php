...
function get_vendor_info($vendor_type, $vendor_type_id)
{
	...
	if($vendor_type=="Hotel Vendor"){
		$sq_hotel = mysqli_fetch_assoc(mysqlQuery("select service_tax_no,state_id from hotel_master where hotel_id='$vendor_type_id'"));
		$vendor_type_arr[0] = $sq_hotel['service_tax_no'];
		$vendor_type_arr[1] = $sq_hotel['state_id'];
	}	
	if($vendor_type=="Transport Vendor"){
		$sq_transport = mysqli_fetch_assoc(mysqlQuery("select service_tax_no,state_id from transport_agency_master where transport_agency_id='$vendor_type_id'"));
		$vendor_type_arr[0] = $sq_transport['service_tax_no'];
		$vendor_type_arr[1] = $sq_transport['state_id'];
	}	
	if($vendor_type=="Car Rental Vendor"){
		$sq_cra_rental_vendor = mysqli_fetch_assoc(mysqlQuery("select service_tax_no,state_id from car_rental_vendor where vendor_id='$vendor_type_id'"));
		$vendor_type_arr[0] = $sq_cra_rental_vendor['service_tax_no'];
		$vendor_type_arr[1] = $sq_cra_rental_vendor['state_id'];
	}
	if($vendor_type=="DMC Vendor"){
		$sq_dmc_vendor = mysqli_fetch_assoc(mysqlQuery("select service_tax_no,state_id from dmc_master where dmc_id='$vendor_type_id'"));
		$vendor_type_arr[0] = $sq_dmc_vendor['service_tax_no'];
		$vendor_type_arr[1] = $sq_dmc_vendor['state_id'];
	}
	if($vendor_type=="Visa Vendor"){
		$sq_visa_vendor = mysqli_fetch_assoc(mysqlQuery("select service_tax_no,state_id from visa_vendor where vendor_id='$vendor_type_id'"));
		$vendor_type_arr[0] = $sq_visa_vendor['service_tax_no'];
		$vendor_type_arr[1] = $sq_visa_vendor['state_id'];
	}
	if($vendor_type=="Passport Vendor"){
		$sq_passport_vendor = mysqli_fetch_assoc(mysqlQuery("select service_tax_no,state_id from passport_vendor where vendor_id='$vendor_type_id'"));
		$vendor_type_arr[0] = $sq_passport_vendor['service_tax_no'];
		$vendor_type_arr[1] = $sq_passport_vendor['state_id'];
	}
	if($vendor_type=="Ticket Vendor"){
		$sq_vendor = mysqli_fetch_assoc(mysqlQuery("select service_tax_no,state_id from ticket_vendor where vendor_id='$vendor_type_id'"));
		$vendor_type_arr[0] = $sq_vendor['service_tax_no'];
		$vendor_type_arr[1] = $sq_vendor['state_id'];
	}
	if($vendor_type=="Train Ticket Vendor"){
		$sq_vendor = mysqli_fetch_assoc(mysqlQuery("select service_tax_no,state_id from train_ticket_vendor where vendor_id='$vendor_type_id'"));
		$vendor_type_arr[0] = $sq_vendor['service_tax_no'];
		$vendor_type_arr[1] = $sq_vendor['state_id'];
	}
	if($vendor_type=="Excursion Vendor"){
		$sq_vendor = mysqli_fetch_assoc(mysqlQuery("select service_tax_no,state_id from site_seeing_vendor where vendor_id='$vendor_type_id'"));
		$vendor_type_arr[0] = $sq_vendor['service_tax_no'];
		$vendor_type_arr[1] = $sq_vendor['state_id'];
	}
	if($vendor_type=="Insurance Vendor"){
		$sq_vendor = mysqli_fetch_assoc(mysqlQuery("select service_tax_no,state_id from insuarance_vendor where vendor_id='$vendor_type_id'"));
		$vendor_type_arr[0] = $sq_vendor['service_tax_no'];
		$vendor_type_arr[1] = $sq_vendor['state_id'];
	}
	if($vendor_type=="Other Vendor"){
		$sq_vendor = mysqli_fetch_assoc(mysqlQuery("select service_tax_no,state_id from other_vendors where vendor_id='$vendor_type_id'"));
		$vendor_type_arr[0] = $sq_vendor['service_tax_no'];
		$vendor_type_arr[1] = $sq_vendor['state_id'];
	}
	if($vendor_type=="Cruise Vendor"){
		$sq_vendor = mysqli_fetch_assoc(mysqlQuery("select service_tax_no,state from cruise_master where cruise_id='$vendor_type_id'"));
		$vendor_type_arr[0] = $sq_vendor['service_tax_no'];
