...
if($depr_type == 'Straight Line'){
	...
	if($purchase_date > $sq_finacial_year['to_date']){
		...
		$sq_depr = mysqli_fetch_assoc(mysqlQuery("select sum(depr_till_date) as depr_till_date from fixed_asset_entries where asset_id ='$asset_name' and asset_ledger ='$asset_ledger'"));
