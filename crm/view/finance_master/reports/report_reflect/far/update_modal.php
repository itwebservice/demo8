...
$entry_id = $_POST['id'];
$sq_asset = mysqli_fetch_assoc(mysqlQuery("select * from fixed_asset_entries where entry_id='$entry_id'"));
?>
<form id="asset_frm_update">

<input type="hidden" name="old_sold_amount" id="old_sold_amount" placeholder="Asset Sold Amount" value="<?= $sq_asset['sold_amount'] ?>" title="Asset Sold Amount">
<input type="hidden" name="entry_id" id="entry_id" value="<?= $entry_id ?>">
<div class="modal fade" id="asset_update_modal" role="dialog" aria-labelledby="myModalLabel" data-backdrop="static" data-keyboard="false">
      ...
      <div class="modal-body">
          <div class="row">
            <div class="col-md-4">
              <select id="asset_type" name="asset_type" onchange="get_assets(this.id,'2')" style="width: 100%" title="Asset Type" disabled>
                <?php 
                $sq_query = mysqli_fetch_assoc(mysqlQuery("select * from fixed_asset_master where entry_id='$sq_asset[asset_id]'"));
                ?>
          ...
          <div class="row mg_tp_20">       
            ...
            <div class="col-md-4">
              ...
              <?php 
              $sq_finacial_year1 = mysqli_fetch_assoc(mysqlQuery("select * from financial_year where financial_year_id ='$sq_asset[depr_interval]'"));
              ...
              <?php 
              $sq_finacial_year = mysqlQuery("select * from financial_year order by financial_year_id desc");
          ...
          <div class="row mg_tp_20">
            ...
            <div class="col-md-4">
              <input type="text" name="sold_amount1" id="sold_amount1" onchange="calculate_profit_loss(this.id,'carrying_amount1','1')" placeholder="Asset Sold Amount" value="<?= $sq_asset['sold_amount'] ?>" title="Asset Sold Amount">
            </div>
            <div class="col-md-4">
          ...
          <div class="row mg_tp_20">
            <div class="col-md-12">
              <div class="div-upload">
                  ...
                  <ul id="files" ></ul>
                  <input type="hidden" id="evidence_url1" name="evidence_url1" value="<?= $sq_asset['evidence_url1'] ?>">
                </div>
            </div>
          </div>    
    ...
    new AjaxUpload(btnUpload, {
      ...
      {  
        ...
        if (! (ext && /^(jpg|png|jpeg|docx|pdf|xlsx)$/.test(ext))){ 
         ...
         error_msg_alert('Only JPG, PNG, Excel, Word, PDF files are allowed');
...
$('#asset_frm_update').validate({
    ...
    submitHandler:function(){
        ...
        var evidence_url1 = $('#evidence_url1').val();
        ...
        $.ajax({
          ...
          data:{ branch_admin_id : branch_admin_id, asset_id : asset_id,  asset_ledger : asset_ledger, purchase_date : purchase_date1, purchase_amount : purchase_amount1, depr_type : depr_type2 , rate_of_depr : rate_of_depr1 ,  depr_interval : depr_interval1 , depr_till_date : depr_till_date1, carrying_amount : carrying_amount1,sold_amount : sold_amount1,profit_loss : profit_loss,remark : remark,evidence_url : evidence_url1,old_sold_amount : old_sold_amount ,entry_id:entry_id},
