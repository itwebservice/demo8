...
function report_reflect()
{
	...
	$.post('report_reflect/trail_balance/report_reflect.php',{ from_date : from_date,branch_admin_id : branch_admin_id,financial_year_id : financial_year_id}, function(data){
		...
		if(data == 0){
			error_msg_alert('Filter till date should be between logged in financial year');
			$('#div_report_trail').html('');
			return false;
		}else{
...
function excel_report()
{
	...
	var excel_url = 'report_reflect/trail_balance/excel_report.php?from_date='+from_date+'&financial_year_id='+financial_year_id+'&branch_admin_id='+branch_admin_id;
	if(from_date != ''){
		$.post(base_url+'view/load_data/finance_date_validation.php', { check_date: from_date }, function(data){
			if(data !== 'valid'){
				error_msg_alert("Filter till date should be between logged in financial year");
				return false;
			}
