...
$sq_journal = mysqli_fetch_assoc(mysqlQuery("select * from journal_entry_master where entry_id='$entry_id'"));
?>
<form id="frm_update">
          ...
          <div class="panel panel-default panel-body app_panel_style feildset-panel">
             <legend>Account Debited</legend>    
                <div class="row"> <div class="col-md-12"> <div class="table-responsive">
                  ...
                  <?php
                       $count = 1; 
                       $debit_query = mysqlQuery("select * from journal_entry_accounts where entry_id='$sq_journal[entry_id]' and type='Debit'");
                        while($row_debit = mysqli_fetch_assoc($debit_query)){
                  ?>
                          <tr>
                              ...
                              <td class="col-md-6"><select name="ledger_id<?= $count ?>" id="ledger_id<?= $count ?>" title="Ledger" class="app_select2" style="width:100%" disabled>    
                                    <?php
                                    $sq_ledger = mysqli_fetch_assoc(mysqlQuery("select * from ledger_master where ledger_id='$row_debit[ledger_id]'"));
                                    ?>
          ...
          <div class="panel panel-default panel-body app_panel_style feildset-panel mg_tp_30">
             <legend>Account Credited</legend>   
                <div class="row"> <div class="col-md-12"> <div class="table-responsive">
                  ...
                  <?php
                       $count = 1; 
                       $credit_query = mysqlQuery("select * from journal_entry_accounts where entry_id='$sq_journal[entry_id]' and type='Credit'");
                        while($row_credit = mysqli_fetch_assoc($credit_query)){
                  ?>
                      <tr>
                          ...
                          <td class="col-md-6"><select name="ledger_id<?= $count ?>" id="ledger_id<?= $count ?>" title="Ledger" class="app_select2" style="width:100%" disabled>                     
                          <?php
                              $sq_ledger = mysqli_fetch_assoc(mysqlQuery("select * from ledger_master where ledger_id='$row_credit[ledger_id]'"));
                              ?>
                                <option value="<?= $sq_ledger['ledger_id'] ?>"><?= $sq_ledger['ledger_name'] ?></option>
                              </select>
                          </td>
    ...
    submitHandler:function(form){
        ...
        for(var i=0; i<rowCount; i++)
        {
          ...
          {  
              ...
              if(ledger_id==""){ $('#btn_update').prop('disabled',false); error_msg +="Debit Ledger is required in row : "+(i+1)+"<br>"; }
              if(amount==""){ $('#btn_update').prop('disabled',false); error_msg +="Debit Amount is required in row : "+(i+1)+"<br>"; }
        ...
        for(var i=0; i<rowCount; i++)
        {
          ...
          {  
              ...
              if(ledger_id==""){ $('#btn_update').prop('disabled',false); error_msg +="Credit Ledger is required in row : "+(i+1)+"<br>"; }
              if(amount==""){ $('#btn_update').prop('disabled',false); error_msg +="Credit Amount is required in row : "+(i+1)+"<br>"; }
        ...
        //Validation
        if(parseFloat(total_debit) != parseFloat(total_credit)){ $('#btn_update').prop('disabled',false); error_msg = "Can not update amount else make it 0"; }  
        
        if(parseFloat(total_debit) != parseFloat(total_old_debit) && parseFloat(total_debit) != '0'){
          $('#btn_update').prop('disabled',false); error_msg = "Can not update amount else make it 0";
        }
