...
<table class="table table-bordered" id="finance_table">
	...
	<tbody>
		<?php 
		$count = 0;
		$sq_finance1 = mysqlQuery("select DISTINCT(module_name) as module_name from finance_transaction_master order by module_name asc");
		while($row_finance1 = mysqli_fetch_assoc($sq_finance1)){

			$sq_finance2 = mysqlQuery("select DISTINCT(module_entry_id) as module_entry_id from finance_transaction_master where module_name='$row_finance1[module_name]'");
			while($row_finance2 = mysqli_fetch_assoc($sq_finance2)){				

				$first_time = true;

				$sq_finance3 = mysqlQuery("select * from finance_transaction_master where module_name='$row_finance1[module_name]' and module_entry_id='$row_finance2[module_entry_id]'");
				while($row_finance3 = mysqli_fetch_assoc($sq_finance3)){

					$sq_gl = mysqli_fetch_assoc(mysqlQuery("select * from gl_master where gl_id='$row_finance3[gl_id]'"));
