...
<table class="table table-bordered" id="tbl_list1" style="margin: 20px 0 !important;">
 	...
 	<tbody>
		...
		<tr class="<?= $bg ?>">
			...
			<td>
				<input type="text" name="status_datec<?= $count ?>" id="status_datec<?= $count ?>" placeholder="Date" value="<?= date('d-m-Y')?>" class="form-control">
			</td>
			<td>
				<?php if($row_transaction['clearance_status']!="Cleared" && $row_transaction['payment_mode'] == 'Credit Card'){ ?>
				<button class="btn btn-info btn-sm" onclick="clearance_status_change1(this, <?= $row_transaction['register_id'] ?>, 'Cleared', 'status_datec<?= $count ?>','<?= $row_transaction['module_name'] ?>',<?= $row_transaction['module_entry_id'] ?>,'',<?= $row_transaction['payment_amount']?>,'<?= $row_transaction['particular']?>','card',<?= $count ?>)" title="Clear"><i class="fa fa-check-square-o"></i></button>
				<?php } ?>
			</td>
			<td>
		...
		<script>
			$('#status_datec<?= $count ?>').datetimepicker({ timepicker:false, format:'d-m-Y' });
		</script>
		<?php } ?>
 	</tbody>
</table>
...
function clearance_status_change1(ele, register_id, status, status_date_id,module_name,module_entry_id,transaction_id,payment_amount,particular,type,count){

	var status_date = $('#status_datec'+count).val();
