...
if($type=="Child With Bed")
{
  ...
  $tour_id=$_GET['tour_id'];
  $children_cost=0;
  $sq=mysqlQuery("select child_with_cost from tour_master where tour_id='$tour_id'");
  if($row=mysqli_fetch_assoc($sq))
  {
...
if($type=="Child Without Bed"){
  ...
  $tour_id=$_GET['tour_id'];
  $children_cost=0;
  $sq=mysqlQuery("select child_without_cost from tour_master where tour_id='$tour_id'");
  if($row=mysqli_fetch_assoc($sq))
  {
...
if($type=="Infant")
{
  ...
  $tour_id=$_GET['tour_id'];
  $infant_cost=0;
  $sq=mysqlQuery("select infant_cost from tour_master where tour_id='$tour_id'");
  if($row=mysqli_fetch_assoc($sq))
  {
...
if($type=="total")
{
  ...
  $sq=mysqlQuery("select children_cost from tour_master where tour_id='$tour_id'");
  if($row=mysqli_fetch_assoc($sq))
  {
