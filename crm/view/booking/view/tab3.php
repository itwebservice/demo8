...
<div class="row">
	<div class="col-md-6">
		<div class="profile_box main_block" style="min-height: 80px;">
			...
			?>
	        <?php
				$sq1 = mysqlQuery("select * from travelers_details where traveler_group_id ='$sq_group_info[traveler_group_id]' and adolescence='Adult' and status='Active'");
				$total_adult = mysqli_num_rows($sq1);

				$sq1 = mysqlQuery("select * from travelers_details where traveler_group_id ='$sq_group_info[traveler_group_id]' and adolescence='Child With Bed' and status='Active' ");
				$total_b_children = mysqli_num_rows($sq1);

				$sq1 = mysqlQuery("select * from travelers_details where traveler_group_id ='$sq_group_info[traveler_group_id]' and adolescence='Child Without Bed' and status='Active' ");
				$total_wb_children = mysqli_num_rows($sq1);

				$sq1 = mysqlQuery("select * from travelers_details where traveler_group_id ='$sq_group_info[traveler_group_id]' and adolescence='Infant' and status='Active' ");
			 ...
			 <div class="profile_box main_block">
		        <div class="row">
		            <div class="col-sm-6 right_border_none_sm" style="border-right: 1px solid #ddd">
						...
						$sq_taxation = mysqli_fetch_assoc(mysqlQuery("select * from taxation_master where taxation_id='$sq_tour_deatils[tour_taxation_id]'"));
					...
					<div class="col-sm-6">
		        	    <span class="main_block">
			        	    <i class="fa fa-angle-double-right cost_arrow" aria-hidden="true"></i>
			        	    <?php echo "<label>Roundoff"." <em>:</em></label> " .$sq_tour_deatils['roundoff']; ?> 
		        	    </span>
		        	    <span class="main_block">
			        	    <i class="fa fa-angle-double-right cost_arrow" aria-hidden="true"></i>
			        	    <?php echo '<label>TCS'.'('.$sq_tour_deatils['tcs_per'].'%)'.'<em>:</em></label> '.$sq_tour_deatils['tcs_tax'] ; ?> 
		        	    </span>
						<span class="main_block">
					      	<i class="fa fa-angle-double-right cost_arrow" aria-hidden="true"></i>
