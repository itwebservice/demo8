...
$sq_inc = mysqli_fetch_assoc(mysqlQuery("select * from inclusions_exclusions_master where inclusion_id='$inclusion_id'"));
      ...
      <div class="modal-body">
          ...
          <div class="row">
            ...
            <div class="col-sm-6 mg_bt_10">
                <?php
                if($sq_inc['for_value'] == 'Group'){
                  $label = 'Group Tour';
                }else if($sq_inc['for_value'] == 'Package'){
                  $label = 'Package Tour';
                }else{
                  $label = $sq_inc['for_value'];
                }
                ?>
              <select name="for_value1" id="for_value1" title="For">
                <option value="<?= $sq_inc['for_value'] ?>"><?= $label ?></option>
                <?php if($sq_inc['for_value']!='Group') {?><option value="Group">Group Tour</option><?php } ?>
                <?php if($sq_inc['for_value']!='Package') {?><option value="Package">Package Tour</option><?php } ?>
                <?php if($sq_inc['for_value']!='Both') {?><option value="Both">Both</option><?php } ?>
                <?php if($sq_inc['for_value']!='B2B Quotation') {?><option value="B2B Quotation">B2B Quotation</option><?php } ?>
              </select>
