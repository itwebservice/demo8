...
<div class="row mg_tp_10"> <div class="col-md-6 col-md-offset-3"> <div class="table-responsive">
	...
	<table class="table table-bordered no-marg-sm">
		...
		<tbody>
			<?php
			$count = 0; 
			$sq_entities = mysqlQuery("select * from checklist_entities where tour_id='$tour_id' AND tour_group_id='$tour_group_id'");
			while($row_entity = mysqli_fetch_assoc($sq_entities))
			{

				$sq_todo_list=mysqlQuery("select * from to_do_entries where entity_id='$row_entity[entity_id]'");

				while($row_query=mysqli_fetch_assoc($sq_todo_list))
				{
					$sq_chk_count = mysqli_num_rows(mysqlQuery("select * from checklist_group_tour where tour_id='$tour_id' AND tour_group_id='$tour_group_id' and entity_id='$row_query[id]'"));
