...
<div class="row"> <div class="col-md-12"> <div class="table-responsive">
	...
	<table class="table table-bordered table-hover mg_bt_0">
		...
		<tbody>
			...
			$today = date('Y-m-d');
			$sq = mysqlQuery("select * from travelers_details where MONTH(birth_date)=MONTH(NOW()) and day(birth_date)>DAYOFMONTH('$today')");
			...
			$sq = mysqlQuery("select * from package_travelers_details where MONTH(birth_date)=MONTH(NOW()) and day(birth_date)>DAYOFMONTH('$today')");
			while($row = mysqli_fetch_assoc($sq)){
