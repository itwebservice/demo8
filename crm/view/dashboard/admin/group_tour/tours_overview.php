...
while($row_tours = mysqli_fetch_assoc($sq_tours)){
	...
	$sq_groups = mysqlQuery("select * from tour_groups where tour_id='$tour_id'");	
	while($row_groups = mysqli_fetch_assoc($sq_groups)){
		...
		$sq_travel_total = mysqli_fetch_assoc(mysqlQuery("select sum(total_travel_expense) as sum from tourwise_traveler_details where tour_id='$tour_id' and tour_group_id='$tour_group_id'"));
		$sq_tour_total = mysqli_fetch_assoc(mysqlQuery("select sum(total_tour_fee) as sum from tourwise_traveler_details where tour_id='$tour_id' and tour_group_id='$tour_group_id'"));

		$travel_total = $sq_travel_total['sum'];
		$tour_total = $sq_tour_total['sum'];
		$total_fee = $travel_total + $tour_total;

		$sq_payment = mysqli_fetch_assoc( mysqlQuery("select sum(amount) as sum from payment_master where tourwise_traveler_id in ( select id from tourwise_traveler_details where tour_id='$tour_id' and tour_group_id='$tour_group_id')") );
