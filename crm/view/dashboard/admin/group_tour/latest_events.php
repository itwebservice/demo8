...
<div class="row tab" id="tab_2"> <div class="col-md-12"> <div class="table-resposive">
	...
	<table class="table table-bordered table-hover mg_bt_0">
		...
		<tbody>
			...
			$count = 0;
			$today = date('Y-m-d');
			$sq_tours = mysqlQuery("select * from tour_groups where status!='Cancel' and date(from_date)>'$today' order by date(from_date) asc limit 5");
			while($row = mysqli_fetch_assoc($sq_tours)){
				$tour_id = $row['tour_id'];
				$sq_tour = mysqli_fetch_assoc(mysqlQuery("select * from tour_master where tour_id='$tour_id'"));
