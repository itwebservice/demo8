        ...
        <table class="table table-hover" style="margin: 0 !important;border: 0;">
            ...
            while($row = mysqli_fetch_assoc($sq_enquiries)){ 
                ...
                while ($enquiry_status=mysqli_fetch_assoc($enquiry_status1)) {
                    ...
                    if($status_count>0){
                        $enquiry_status2 = mysqli_fetch_assoc(mysqlQuery("select * from enquiry_master_entries where entry_id=(select max(entry_id) from enquiry_master_entries where enquiry_id='$row[enquiry_id]') "));
                        ...
                        if($enquiry_status2['followup_status']!=$enquiry_status_filter){
                            continue;
                        }
                        }
                    }
                    ...
                    if($status_count1==1){
                        ...
                        $enquiry_status3 = mysqli_fetch_assoc(mysqlQuery("select * from enquiry_master_entries where entry_id=(select max(entry_id) from enquiry_master_entries where enquiry_id='$row[enquiry_id]') "));
