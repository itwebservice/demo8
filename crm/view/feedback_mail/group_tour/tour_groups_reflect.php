...
$sq_tour_info = mysqli_fetch_assoc(mysqlQuery("select tour_name from tour_master where tour_id='$tour_id'"));
$count = 0;
?>
	...
	<table class="table table-bordered" id="feedback_grp_tour_id" style="margin: 20px 0 !important;">
		...
		<tbody>
			...
			while($row_tour_groups = mysqli_fetch_assoc($sq_tour_groups)){
				...
				$sq_tour_info =  mysqli_fetch_assoc(mysqlQuery("select * from tour_master where tour_id = '$row_tour_groups[tour_id]'"));
				?>
				<tr>
					<td><?= ++$count ?></td>
