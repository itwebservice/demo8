...
$query = mysqli_fetch_assoc(mysqlQuery("SELECT popular_activities FROM `b2c_settings` where setting_id='1'"));
...
<form id="section_activities">
    ...
    <div class="row"> 
        <div class="col-md-4"> <label class="alert-danger">For saving activity keep checkbox selected!</label> </div>
    ...
    <table id="tbl_activities" name="tbl_activities" class="table border_0 table-hover no-marg">
        ...
        ?>
            <tr>
                ...
                <td><select name="city_name-1" id="city_name-1" onchange="excursion_dynamic_reflect(this.id)" style="width:100%" class="form-control city_name_u" title="Select City">
                        <?php if($city_id!=''){ ?>
                        <option value="<?= $sq_city['city_id'] ?>" selected="selected"><?= $sq_city['city_name'] ?></option>
                        <?php } ?>
                    </select>
                </td>
                <td><select id="exc-1" name="exc-1" class="form-control" style="width:100%" title="Select Activity">
        ...
        <?php }
            ...
            for($i=0;$i<sizeof($popular_activities);$i++){
                ...
                $sq_ex = mysqli_fetch_assoc(mysqlQuery("select entry_id,excursion_name from excursion_master_tariff where entry_id='$exc_id'"));
                ?>
                <tr>
                    ...
                    <td><select name="city_name-1<?=$i?>_u" id="city_name-1<?=$i?>_u" onchange="excursion_dynamic_reflect(this.id)" style="width:100%" class="form-control city_name_u" title="Select City">
                            <?php if($city_id!=''){ ?>
                            <option value="<?= $sq_city['city_id'] ?>" selected="selected"><?= $sq_city['city_name'] ?></option>
                            <?php } ?>
                        </select>
                    </select></td>
                    <td><select id="exc-1<?=$i?>_u" name="exc-1<?=$i?>_u" class="form-control" style="width:100%" title="Select Activity">
