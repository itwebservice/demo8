...
$query = mysqli_fetch_assoc(mysqlQuery("SELECT fit_tours FROM `b2c_settings` where setting_id='1'"));
$fit_tours = ($query['fit_tours']!='' && $query['fit_tours']!='null') ? json_decode($query['fit_tours']): [];
?>
<form id="section_package">
    <legend>Active & Inactive Package Tours</legend>
    <div class="row"> 
        <div class="col-md-4"> <label class="alert-danger">For saving package keep checkbox selected!</label> </div>
    ...
    <table id="tbl_dest_packages_datewise" name="tbl_dest_packages_datewise" class="table border_0 table-hover">
        ...
        <tr>
            ...
            <td><select class="form-control" name="validity" id="validity-1" title="Select Status">
  ...
  submitHandler:function(form){
    ...
    for(var i=0; i<rowCount; i++){
        ...
        if(row.cells[0].childNodes[0].checked){
            ...
            if(validity==""){ error_msg_alert("Select status at row "+(i+1)); return false; }
