	...
	<div class="row">
		<div class="col-md-4 col-md-offset-4 col-sm-6 col-sm-offset-3 col-xs-12">
			<select name="booking_id" id="booking_id" style="width:100%" title="Select Booking" onchange="content_reflect()">
				...
				<?php 
				$sq_b2b = mysqlQuery("select * from b2c_sale where booking_id in ( select booking_id from b2c_sale where status='Cancel' )  order by booking_id desc");
				while($row_b2b = mysqli_fetch_assoc($sq_b2b)){
					...
					?>
					<option value="<?= $row_b2b['booking_id'] ?>"><?= get_b2c_booking_id($row_b2b['booking_id'],$year).' : '.$sq_customer['first_name'].' '.$sq_customer['last_name'] ?></option>
					<?php
				}
		        ?>
		    </select>
