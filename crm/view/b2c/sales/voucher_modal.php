...
$count = 0;
$sq_count = mysqli_num_rows(mysqlQuery("select * from b2c_transport_voucher where booking_id='$booking_id'"));
$sq_count_p = mysqli_num_rows(mysqlQuery("select * from custom_package_transport where package_id='$package_id'"));
$sq = mysqli_fetch_assoc( mysqlQuery("select * from b2c_transport_voucher where booking_id='$booking_id'") );
$sq_transport = mysqli_fetch_assoc( mysqlQuery("select * from b2c_transport_voucher_entries where booking_id='$booking_id'") );
?>
	...
	<div class="modal-dialog" role="document">
		...
		<div class="modal-body">
			<div class="panel panel-default panel-body">
				...
				if($sq_count_p != 0){
					$countVehicle = 0;
					$sq_package = mysqlQuery("select * from custom_package_transport where package_id='$package_id'");
					while($row_entry1 = mysqli_fetch_assoc($sq_package)){

						$count++;
						$q_transport = mysqli_fetch_assoc(mysqlQuery("select * from b2b_transfer_master where entry_id='$row_entry1[vehicle_name]'"));
						
						$sq_count_v = mysqli_fetch_assoc(mysqlQuery("select * from b2c_transport_voucher_entries where booking_id='$booking_id' and transport_bus_id='$row_entry1[vehicle_name]'"));
						if($sq_count_v == 0){
							...
							$entriesDriver = array();
							$sq_entry_n = mysqlQuery("select * from b2c_transport_voucher_entries where booking_id='$booking_id'");
...
$(function(){
	$('#frm_service_voucher').validate({
		...
		submitHandler:function(form, event){
				...
				$.ajax({
					type:'post',
					url:base_url+'controller/b2c_settings/b2c/transport_service_voucher_save.php',
						...
						else
						{
							if(msg!=''){
								...
								callback: function(data1){
										if(data1=="yes"){
											var url1 = base_url+'model/app_settings/print_html/voucher_html/b2c_voucher.php?booking_id='+booking_id;
											loadOtherPage(url1);
