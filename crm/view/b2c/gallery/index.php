...
$query = mysqli_fetch_assoc(mysqlQuery("SELECT gallery FROM `b2c_settings` where setting_id='1'"));
...
<form id="section_gallery">
    ...
    <div class="row mg_bt_20">
        <div class="col-md-3">
            ...
            <?php 
                if($dest_id1!=''){
                    $sq_dest1 = mysqli_fetch_assoc(mysqlQuery("select * from destination_master where dest_id='$dest_id1'"));
                ?>
                <option value="<?= $sq_dest1['dest_id'] ?>"><?= $sq_dest1['dest_name'] ?></option>
                <?php } ?>
                <option value="">Select Destination</option>
                <?php
                $sq_dest = mysqlQuery("select * from destination_master where status='Active'");
                while($row_dest = mysqli_fetch_assoc($sq_dest)){
                ?>
