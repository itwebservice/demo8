...
$sq_package_members = mysqli_num_rows(mysqlQuery("select traveler_id from package_travelers_details where booking_id='$booking_id'"));
...
if($sq_package_members > 0)
{
    ...
    $sq_members = mysqlQuery("select * from package_travelers_details where booking_id = '$booking_id'");
    while($row_members = mysqli_fetch_assoc($sq_members))
    {
        $pdf->SetX(10);
...
$pdf->SetY($pdf->GetY()+0); 
    ...
    if($sq_customer['type'] == 'Corporate'||$sq_customer['type'] == 'B2B') { $company_name = $sq_customer['company_name']; }
...
$sq_count = mysqli_num_rows(mysqlQuery("select id from package_hotel_accomodation_master where booking_id='$booking_id'"));
...
if($sq_count > 0)
{
    ...
    $y_pos1+=5;
    $pdf->SetXY(10, $y_pos1);
    $pdf->MultiCell(185, 10, 'ACCOMMODATION DETAILS', 1,'C');
...
$sq_entry = mysqli_fetch_assoc(mysqlQuery("select * from package_tour_booking_master where booking_id='$booking_id'"));
    ...
    $sq_agency = mysqli_fetch_assoc(mysqlQuery("select * from transport_agency_master where transport_agency_id='$transport_agency_id'"));
    $sq_bus_name = mysqli_fetch_assoc(mysqlQuery("select * from b2b_transfer_master where entry_id='$transport_bus_id'"));
...
$pdf->SetY($pdf->GetY()+0); 
    ...
    $pdf->SetX(10);
    $sq_itinerary_entry = mysqlQuery("select * from package_tour_schedule_master where booking_id='$booking_id'");
    while($row_itinerary_entry = mysqli_fetch_assoc($sq_itinerary_entry)){
...
//Advance Payment
$payment_count= mysqli_num_rows(mysqlQuery("select * from package_payment_master where booking_id='$booking_id' and payment_for='tour'and advance_status='true' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
if($payment_count > 0)
{
    $payment_details = mysqli_fetch_assoc(mysqlQuery("select * from package_payment_master where booking_id='$booking_id' and payment_for='tour'and advance_status='true' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
...
//**Tour Payment Sum
$sq_tour_payment = mysqli_fetch_assoc(mysqlQuery("select sum(amount) as sum_payment from package_payment_master where booking_id='$booking_id' and payment_for='tour' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
...
$travel_payment_details = mysqli_fetch_assoc(mysqlQuery("select * from package_payment_master where booking_id='$booking_id' and payment_for='traveling' and advance_status='true' and clearance_status!='Pending' and clearance_status!='Cancelled'"));

$travel_payment_count= mysqli_num_rows(mysqlQuery("select  * from package_payment_master where booking_id='$booking_id' and payment_for='traveling' and advance_status='true' and clearance_status!='Pending' and clearance_status!='Cancelled'"));

$total_payment1 = mysqli_fetch_assoc(mysqlQuery("select sum(amount) as sum_payment from package_payment_master where booking_id='$booking_id' and payment_for='traveling' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
...
if($sq_train > 0 || $sq_air > 0)
{
    ...
    if($sq_train>0)
    {
        ...
        $sq_train_details = mysqlQuery("select * from package_train_master where booking_id='$booking_id'");
    ...
    if($sq_air>0)
    {
        ...
        $sq_air_details = mysqlQuery("select * from package_plane_master where booking_id='$booking_id'");
