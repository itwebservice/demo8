...
$q_transport = mysqli_fetch_assoc(mysqlQuery("select * from b2b_transfer_master where entry_id='$package_booking_info[transport_bus_id]'"));
$pdf->SetX(27);
$pdf->SetFont('Arial','',9);
...
if($sq_cruise>0)
{
    ...
    $sq_cruise_details = mysqlQuery("select * from package_cruise_master where booking_id='$booking_id'");
    while($row_cruise_details = mysqli_fetch_assoc($sq_cruise_details))
    {
        $pdf->SetX(27);
        $pdf->SetFont('Arial','',10);
        $pdf->Row(array(date("d-m-Y H:i", strtotime($row_cruise_details['dept_datetime'])), date("d-m-Y H:i", strtotime($row_cruise_details['arrival_datetime'])),$row_cruise_details['route'],$row_cruise_details['cabin'],$row_cruise_details['sharing'],$row_cruise_details['seats']));
