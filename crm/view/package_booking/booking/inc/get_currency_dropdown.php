...
$readonly = ($quotation_id != '0') ? 'disabled' : '';
if($quotation_id!=''){

?>
<select class="form-control" name="currency_code" id="currency_code" title="Currency" style="width:100%" data-toggle="tooltip" required <?=$readonly?>>
    <?php
    if(($quotation_id != '0')){

        $sq_currencyd = mysqli_fetch_assoc(mysqlQuery("SELECT `currency_code` FROM `package_tour_quotation_master` WHERE quotation_id='$quotation_id'"));
