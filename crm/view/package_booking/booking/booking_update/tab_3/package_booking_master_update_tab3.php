 ...
 <!--=======Header panel======-->
  ...
  <!--=======Header panel end======-->
    ...
    <div class="">
                ...
                <?php
                        ...
                        <tr>
                            ...
                            <td><select id="hotel_name1<?php echo $count_ht."_h" ?>" name="hotel_name1<?php echo $count_ht."_h" ?>" style="width:150px" title="Select Hotel Name">
                                ...
                                ?>
                                <option value="<?php echo $rows['hotel_id'] ?>" <?= $selected ?>><?= $rows['hotel_name'] ?></option>   
                                <?php } ?>
                                  </select></td>
                            <td><input type="text" id="txt_hotel_from_date<?php echo $count_ht."_h" ?>" placeholder="Check-In DateTime" onchange="get_to_datetime(this.id,'txt_hotel_to_date<?php echo $count_ht.'_h' ?>')" value="<?php echo date("d-m-Y H:i", strtotime($row_hotel_acc['from_date'])) ?>" title="Check-In DateTime" style="width:170px"></td>
                            <td><input type="text" id="txt_hotel_to_date<?php echo $count_ht."_h" ?>" placeholder="Check-Out DateTime" onchange="validate_validDatetime('txt_hotel_from_date<?php echo $count_ht.'_h' ?>' ,'txt_hotel_to_date<?php echo $count_ht.'_h' ?>')" value="<?php echo date("d-m-Y H:i", strtotime($row_hotel_acc['to_date'])) ?>" title="Check-Out DateTime" style="width:170px"></td>
                        ...
                        <script>
                            $('#txt_hotel_from_date<?php echo $count_ht."_h" ?>, #txt_hotel_to_date<?php echo $count_ht."_h" ?>').datetimepicker({  format:'d-m-Y H:i' });
                        </script>
                    <?php } ?>
                </table>
                ...
                <div class="table-responsive">
                    ...
                    <?php
                        ...
                        <tr>
                            ...
                            <td><input type="text" id="txt_tsp_from_date<?= $count_tt ?>-u" name="txt_tsp_from_date<?= $count_tt ?>-u" placeholder="Start Date" title="Start Date" style="width: 170px" value="<?= get_date_user($row_trans_acc['transport_from_date']) ?>" class="form-control app_datepicker" onchange="get_to_date(this.id,'txt_tsp_end_date<?= $count_tt ?>-u')"></td>
                            <td><input type="text" id="txt_tsp_end_date<?= $count_tt ?>-u" name="txt_tsp_end_date<?= $count_tt ?>-u" placeholder="End Date" title="End Date" style="width: 170px" value="<?= get_date_user($row_trans_acc['transport_end_date']) ?>" class="form-control app_datepicker" onchange="validate_validDate('txt_tsp_from_date<?= $count_tt ?>-u','txt_tsp_end_date<?= $count_tt ?>-u')"></td>
                        ...
                        <script>
                        $( "#txt_tsp_from_date<?= $count_tt ?>-u,#txt_tsp_end_date<?= $count_tt ?>-u,#txt_tsp_to_date<?= $count_tt ?>-u").datetimepicker({ timepicker:false,format: "d-m-Y"  });
                        </script>
                        <?php } ?>
                </table>
                </div>
        ...
        <div class="row" style="margin-top: 5px"> <div class="col-md-12 text-right">
            <button type="button" class="btn btn-excel btn-sm" onClick="addRow('tbl_package_exc_infomration');city_lzloading('.city_name');" title="Add Row"><i class="fa fa-plus"></i></button>
        </div> </div>
        <div class="row mg_tp_10">
            <div class="col-xs-12"> 
                <div class="table-responsive">
                            ...
                            <tr>
                                ...
                                <td><select id="excursion-1<?= $count_et ?>" class="form-control" title="Activity Name" name="excursion-1<?= $count_et ?>" style="width:200px">
                                    <option value="">*Activity Name</option>
                                        ...
                                        <option value="<?php echo $rows['entry_id'] ?>"  <?= $selected ?>><?php echo $rows['excursion_name'] ?></option>
                                        <?php } ?>
                                    </select></td>
                                ...
                                <td><input type="number" id="adult-1<?= $count_et ?>" name="adult-1<?= $count_et ?>" placeholder="Adult(s)" title="Adult(s)" style="width:100px" onchange="validate_balance(this.id);" value="<?= $row_exc_acc['adult'] ?>"></td>
                                <td><input type="number" id="child-1<?= $count_et ?>" name="child-1<?= $count_et ?>" placeholder="Child With-Bed" title="Child With-Bed" style="width:150px" onchange="validate_balance(this.id);" value="<?= $row_exc_acc['chwb'] ?>" ></td>
                                <td><input type="number" id="childwo-1<?= $count_et ?>" name="childwo-1<?= $count_et ?>" placeholder="Child Without-Bed" title="Child Without-Bed" style="width:150px" onchange="validate_balance(this.id);" value="<?= $row_exc_acc['chwob'] ?>" ></td>
                                <td><input type="number" id="infant-1<?= $count_et ?>" name="infant-1<?= $count_et ?>" placeholder="Infant(s)" title="Infant(s)" style="width:100px" onchange="validate_balance(this.id);" value="<?= $row_exc_acc['infant'] ?>" ></td>
                                <td style="display:none"><input type="text" value="<?php echo $row_exc_acc['entry_id'] ?>"></td>
                            </tr>
                            <script>
                                $('#city_name-1<?= $count_et ?>').select2();
                                $('#exc_date-1<?= $count_et ?>').datetimepicker({  format:'d-m-Y H:i' });
                            </script>
                        <?php } ?>
                    </table>
                </div>
...
<div class="panel panel-default main_block bg_light pad_8 text-center mg_tp_20 mg_bt_150">
