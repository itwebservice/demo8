...
<div class="row mg_bt_30"> <div class="col-xs-12"> <div class="table-responsive">
    ...
    ?>
        <tr>
            ...
            <td><input type="text" id="<?php echo 'txt_train_date'.$count_t.'t' ?>" name="<?php echo 'txt_train_date'.$count_t.'t' ?>" class="app_datetimepicker" onchange="validate_pastDate(this.id)" placeholder="Departure Date & Time" title="Departure Date & Time" value="<?php echo date("d-m-Y H:i", strtotime($row_train_details['date'])) ?>" style="width: 156px"/></td>
            ...
            <td><select id="txt_train_from_location<?= $count_t ?>t" onchange="validate_location('txt_train_from_location<?= $count_t ?>t' , 'txt_train_to_location<?= $count_t ?>t');" name="txt_train_from_location<?= $count_t ?>t" class="app_select2" style="width:120px">
                ...
                <?php 
                    $sq_city = mysqlQuery("select city_name from city_master");
            ...
            <td style="width: 30px;"><input type="text" id="<?php echo 'txt_train_total_seat'.$count_t.'t' ?>" name="<?php echo 'txt_train_total_seat'.$count_t.'t' ?>" placeholder="Total Seats" title="Total Seats" maxlength="2" onchange="validate_balance(this.id);" value="<?php echo $row_train_details['seats'] ?>"/></td>
            ...
            <td style="width: 130px;"><select id="<?php echo 'cmb_train_class'.$count_t.'t' ?>" title="class" name="<?php echo 'cmb_train_class'.$count_t.'t' ?>"> 
