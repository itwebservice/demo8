...
<div class="row mg_bt_30"> <div class="col-xs-12"> <div class="table-responsive">
    ...
    ?>
        <tr>
            ...
            <td><input type="text" id="cruise_departure_date<?= $count_t ?>t" onchange="get_to_datetime(this.id,'cruise_arrival_date<?= $count_t ?>t')" name="cruise_departure_date<?= $count_t ?>t" placeholder="Departure Date & Time" title="Departure Date & Time" class="app_datetimepicker" value="<?php echo get_datetime_user($row_cruise_details['dept_datetime']); ?> " style="width: 150px;"></td>
            <td><input type="text" id="cruise_arrival_date<?= $count_t ?>t" onchange="validate_validDatetime('cruise_departure_date<?= $count_t ?>t',this.id)" name="cruise_arrival_date<?= $count_t ?>t" placeholder="Arrival Date & Time" title="Arrival Date & Time" class="app_datetimepicker" value="<?php echo get_datetime_user($row_cruise_details['arrival_datetime']); ?> " style="width: 150px;"></td>
        ...
        <script type="text/javascript">
            $('#cruise_departure_date<?= $count_t ?>t,#cruise_arrival_date<?= $count_t ?>t').datetimepicker({ format: "d-m-Y H:i"  });
        </script>
        <?php } ?>
    </table>
</div>  </div> </div>   
