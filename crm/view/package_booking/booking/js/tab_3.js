...
$('#txt_tsp_from_date,#txt_tsp_end_date, #txt_tsp_to_date').datetimepicker({ timepicker:false, format:'d-m-Y' });
    ...
    rules:{
      ...
      for(var i=0; i<rowCount; i++)
      {
        ...
        if(row.cells[0].childNodes[0].checked)
        {
          ...
          if($("option:selected", $("#"+row.cells[5].childNodes[0].id)).parent().attr('value')==""){ error_msg_alert("Transport Pickup location in row-"+(i+1)+" is required<br>"); return false; }
          if(row.cells[6].childNodes[0].value==""){ error_msg_alert("Transport Drop location in row-"+(i+1)+" is required<br>"); return false; }
          if(row.cells[7].childNodes[0].value==""){ error_msg_alert("Vehicle count in row-"+(i+1)+" is required<br>"); return false; }
