...
$('#customer_id_p,#quotation_id, #country_name,#currency_code').select2();
		...
		submitHandler: function (form) {
			...
			if (quotation_id == 0) {
				...
				for (var i = 0; i < table.rows.length; i++) {
					var row = table.rows[i];
					row.cells[2].childNodes[0].value = from_date + " 00:00";
					row.cells[8].childNodes[0].value = from_date + " 00:00";
				}
				//Cruise info
		...
		$.ajax({
			...
			success: function (result) {
				...
				for (var i = 0; i < transport_info_arr.length; i++) {
					...
					row.cells[7].childNodes[0].value = transport_info_arr[i]['vehicle_count'];
		...
		$.ajax({
			...
			success: function (result) {
				...
				for (var i = 0; i < transport_info_arr.length; i++) {
					...
					row.cells[7].childNodes[0].value = 1;
