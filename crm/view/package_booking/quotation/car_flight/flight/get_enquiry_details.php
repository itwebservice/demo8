...
// 		$sector_from = mysqli_fetch_assoc(mysqlQuery("select airport_code,airport_name from airport_master where airport_id = ".$enquiry_content_arr2['value']));
// 		$sq_enq['sector_from'] = $sector_from['airport_name'];
// 		$sq_enq['sector_from_added'] = $sector_from['airport_name']."(".$sector_from['airport_code'].")";
// 		$sq_enq['from_city_name'] = mysqli_fetch_assoc(mysqlQuery("select city_id from airport_master where airport_id = $enquiry_content_arr2[value]"));
	
// 	}
// 	if($enquiry_content_arr2['name']=="sector_to"){
// 		$sector_to = mysqli_fetch_assoc(mysqlQuery("select airport_code,airport_name from airport_master where airport_id = ".$enquiry_content_arr2['value']));
// 		 $sq_enq['sector_to'] = $sector_to['airport_name'];
// 		 $sq_enq['sector_to_added'] = $sector_to['airport_name']."(".$sector_to['airport_code'].")";
// 		 $sq_enq['to_city_name'] = mysqli_fetch_assoc(mysqlQuery("select city_id from airport_master where airport_id = $enquiry_content_arr2[value]"));
		
// 		}
// 	if($enquiry_content_arr2['name']=="preffered_airline"){ $sq_enq['preffered_airline_id'] = $enquiry_content_arr2['value'];
// 	$airline_name = mysqli_fetch_assoc(mysqlQuery("select airline_name,airline_code from airline_master where airline_id = ".$enquiry_content_arr2[value]));
