...
<?php
				...
				<div class="panel panel-default panel-body app_panel_style">
					<div class="row mg_bt_20_sm_xs">
					    <div class="col-xs-12">
					        ...
					        <table id="tbl_package_tour_quotation_dynamic_costing" name="tbl_package_tour_quotation_dynamic_costing" class="table no-marg border_0" disabled>
							<?php
							$count = 0;
							...
							while($row_q_costing = mysqli_fetch_assoc($sq_q_costing)){
								$count++;
								$sq_package = mysqli_fetch_assoc(mysqlQuery("select * from custom_package_master where package_id = '$row_q_costing[package_id]'"));
							...
							?>
					            <tr>
					                <td class="header_btn hidden" style="display:none;"><small>&nbsp;</small><input class="css-checkbox" id="chk_costing<?=$count?>" type="checkbox" checked disabled><span class="css-label" for="chk_costing1"></span></td>

					                <td class="header_btn hidden" style="display:none;"><small>&nbsp;</small><input maxlength="15" value="1" type="text" name="username" placeholder="Sr. No." class="form-control" disabled /><span>SR.NO</span></td>
					                ...
					                <td class="header_btn"><small>&nbsp;</small><input type="text" id="tour_cost-<?=$count?>" name="tour_cost" placeholder="Hotel Cost" title="Hotel Cost" onchange="validate_balance(this.id);quotation_cost_calculate1(this.id);" value="<?php echo $row_q_costing['tour_cost']; ?>" style="width:100px"><span>Hotel Cost</span></td>    

								    <td class="header_btn"><small>&nbsp;</small><input type="text" id="transport_cost-<?=$count?>" name="transport_cost" placeholder="Transport Cost" title="Transport Cost" onchange="validate_balance(this.id);quotation_cost_calculate1(this.id)" value="<?php echo $row_q_costing['transport_cost']; ?>" style="width:100px"><span>Transport Cost</span></td>  
									
								    <td class="header_btn"><small>&nbsp;</small><input type="text" id="excursion_cost-<?=$count?>" name="excursion_cost" onchange="quotation_cost_calculate1(this.id); validate_balance(this.id)"  placeholder="Activity Cost" title="Activity Cost" value="<?= $row_q_costing['excursion_cost'] ?>" style="width:150px"><span>Activity Cost</span></td>

								    <td class="header_btn"><small id="basic_show-" style="color:#000000">&nbsp;</small><input type="<?= $add_class1 ?>" id="basic_amount-<?=$count?>" name="basic_amount" onchange="quotation_cost_calculate1(this.id);get_business(this.id,'true',true);validate_balance(this.id)"  placeholder="Basic Amount" title="Basic Amount" style="width:100px" value="<?= $row_q_costing['basic_amount'] ?>" readonly><span>Basic Amount</span></td>

									<td class="header_btn"><small id="service_show-" style="color:#000000">&nbsp;</small><input type="<?= $add_class1 ?>" id="service_charge-<?=$count?>" name="service_charge" onchange="get_business(this.id,'false');quotation_cost_calculate1(this.id); validate_balance(this.id)" style="width:150px"  placeholder="Service charge" title="Service charge" value="<?= $row_q_costing['service_charge'] ?>"><span>Service charge</span></td>	

					    			<td class="header_btn"><small>&nbsp;</small><input type="text" id="service_tax_subtotal-<?=$count?>" name="service_tax_subtotal" readonly placeholder="Tax Amount" title="Tax Amount" value="<?= $row_q_costing['service_tax_subtotal'] ?>" style="width:150px"><span>Tax Amount</span></td>

					    			<td class="header_btn"><small>&nbsp;</small><input type="text" id="total_tour_cost-<?=$count?>" class="amount_feild_highlight text-right" name="total_tour_cost" placeholder="Total Cost" title="Total Cost" value="<?= $row_q_costing['total_tour_cost'] ?>" style="width: 100px;"  readonly><span>Total Cost</span></td>

					                <td class="header_btn"><small>&nbsp;</small><input type="text" id="package_name1-<?=$count?>" name="package_name1" placeholder="Package Name" title="Package Name" value="<?php echo $package_id; ?>" style="display: none" readonly></td> 
		...
		<div class="panel panel-default panel-body app_panel_style">			
		    <!-- Adult & child cost -->
        		...
        		while($row_q_costing1 = mysqli_fetch_assoc($sq_q_costing1)){
        			...
        			$sq_package = mysqli_fetch_assoc(mysqlQuery("select * from custom_package_master where package_id = '$row_q_costing1[package_id]'"));
					$id = $row_q_costing1['id'];
					?>
					<div class="row mg_tp_10">
						<div class="col-md-3">
							<span>Package Type</span>
							<input type="text" id="package_type1" onchange="validate_balance(this.id);" name="package_type" placeholder="Package Type" title="Package Type" value="<?= $row_q_costing1['package_type'] ?>" readonly>
						</div>
						<div class="col-md-2">
							<span>Adult Cost</span>
							<input type="text" id='adult_cost1<?= $countp ?>' onchange="validate_balance(this.id);" name="adult_cost1" placeholder="Adult Cost" title="Adult Cost" value="<?= $row_q_costing1['adult_cost'] ?>">
						</div>
						<div class="col-md-2">
							<span>Child with Bed Cost</span>
							<input type="text" id="child_with1<?= $countp ?>" onchange="validate_balance(this.id);" name="child_with1" placeholder="Child with Bed Cost" title="Child with Bed Cost" value="<?= $row_q_costing1['child_with'] ?>">
						</div>
						<div class="col-md-2">
							<span>Child w/o Bed Cost</span>
							<input type="text" id="child_without1<?= $countp ?>" onchange="validate_balance(this.id);" name="child_without1" placeholder="Child w/o Bed Cost" title="Child w/o Bed Cost" value="<?= $row_q_costing1['child_without'] ?>">
						</div>
						<div class="col-md-2">
							<span>Infant Cost</span>
							<input type="text" id="infant_cost1<?= $countp ?>" onchange="validate_balance(this.id);" name="infant_cost1" placeholder="Infant Cost" title="Infant Cost" value="<?= $row_q_costing1['infant_cost'] ?>">
						</div>
	...
	<div class="row mg_tp_20">
		<div class="col-md-3 col-sm-6 col-sm-12 mg_bt_10">
			<input type="number" id="discount1" name="discount" placeholder="Discount" title="Discount" value="<?= $sq_quotation['discount'] ?>" />
		</div>
...
function get_business(id, flag, change = false){
	...
	var offset = id.split('-');
	get_auto_values('quotation_date','basic_amount-'+offset[1],'payment_mode','service_charge-'+offset[1],'markup','update',flag,'markup','discount',offset[1],change);
}
...
e.preventDefault();
		...
		for(var i=0; i<rowCount; i++){
			...
			if(row.cells[16] && row.cells[16].childNodes[0]){
				var hotel_id1 = row.cells[16].childNodes[0].value;
			}
			else{
				var hotel_id1 = '';
			}
		...
		for(var i=0; i<rowCount; i++){
			...
			var transport_cost = row.cells[8].childNodes[0].value;
			...
			var transport_id = row.cells[13].childNodes[0].value; 
			}else{
			var transport_id = '';
			}
		...
		for(var i=0; i<rowCount; i++){
			...
			var service_tax_subtotal = row.cells[8].childNodes[1].value;   
			var total_tour_cost = row.cells[9].childNodes[1].value;   
			var package_name3 = row.cells[10].childNodes[1].value;  
			var costing_id = row.cells[11].childNodes[0].value;  
