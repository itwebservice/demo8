<div class="row">
    <div class="col-xs-12">
        ...
        <table id="tbl_package_hotel_master" name="tbl_package_hotel_master" class="table mg_bt_0 table-bordered mg_bt_10">
        	...
        	<?php 
        	$sq_hotel_count = mysqli_num_rows(mysqlQuery("select * from group_tour_hotel_entries where tour_id='$sq_quotation[tour_group_id]'"));
        	if($sq_hotel_count==0){
        		?>
				<tr>
        		...
        		$count = 0;
        		$sq_q_hotel = mysqlQuery("select * from group_tour_hotel_entries where tour_id='$sq_quotation[tour_group_id]'");
        		while($row_q_hotel = mysqli_fetch_assoc($sq_q_hotel))
        		{
        			$city = mysqli_fetch_assoc(mysqlQuery("select city_name from city_master where city_id = ".$row_q_hotel['city_id']));
                    $hotel = mysqli_fetch_assoc(mysqlQuery("select hotel_name from hotel_master where hotel_id = ".$row_q_hotel['hotel_id']));
        			$count++;
        			?>
					<tr>
						...
						<td><input type="text" id="hotel_type<?= $count ?>" name="hotel_type1" placeholder="*Hotel Category" class="form-control" title="Hotel Category" value="<?= $row_q_hotel['hotel_type']?>" readonly></td>
