...
if($sq_train_count>0){
	...
	while($row_train = mysqli_fetch_assoc($sq_train)){
		$pdf->setX(28);	
		$pdf->Row(array($row_train['from_location'], $row_train['to_location'], $row_train['class'],date('d-m-Y H:i', strtotime($row_train['departure_date']))  , date('d-m-Y H:i', strtotime($row_train['arrival_date']))));
		//Adding new page if end of page is found
        if($pdf->GetY()+20>$pdf->PageBreakTrigger)
        {
        	$pdf->AddPage($pdf->CurOrientation);
...
if($sq_plane_count>0){
	...
	while($row_plane = mysqli_fetch_assoc($sq_plane)){
	    ...
	    $y_pos = $pdf->getY();
		$pdf->setXY(28, $y_pos);
		$pdf->Row(array($row_plane['from_location'], $row_plane['to_location'],$sq_airline['airline_name'].' ('.$sq_airline['airline_code'].')', $row_plane['class'], date('d-m-Y H:i', strtotime($row_plane['dapart_time'])), date('d-m-Y H:i', strtotime($row_plane['arraval_time']))));
