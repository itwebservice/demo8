...
$sq_train = mysqlQuery($query);
	...
	while($row_train = mysqli_fetch_assoc($sq_train)){		
		$sq_city = mysqli_fetch_assoc(mysqlQuery("select * from city_master where city_id='$row_train[from_city]'"));
		$sq_city1 = mysqli_fetch_assoc(mysqlQuery("select * from city_master where city_id='$row_train[to_city]'"));
