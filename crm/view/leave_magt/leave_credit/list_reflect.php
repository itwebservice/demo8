	...
	<tbody>
		...
		while($row_credit = mysqli_fetch_assoc($sq_credit)){
			...
			<tr class="<?= $bg ?>">
			    ...
			    <td><?= ($row_credit['casual']<0)?0:$row_credit['casual'] ?></td> 
				<td><?= ($row_credit['paid']<0)?0:$row_credit['paid'] ?></td>
				<td><?= ($row_credit['medical']<0)?0:$row_credit['medical'] ?></td>
				<td><?= ($row_credit['maternity']<0)?0:$row_credit['maternity'] ?></td>
