...
$sq_payment = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum from train_ticket_payment_master where payment_id in ('$payment_id_arr1')"));
...
{
	$sq = mysqlQuery("select * from train_ticket_payment_master where payment_id='$payment_id_arr[$i]'");
