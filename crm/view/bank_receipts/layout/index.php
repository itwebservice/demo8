...
$acc_no = mysqli_fetch_assoc(mysqlQuery("SELECT `account_no` FROM `bank_master` WHERE `bank_id` = '$bank_id_arr1[0]'"));
$acc_no_arr = str_split($acc_no['account_no']);
...
for($cells = 0; $cells < 16; $cells++){
	$pdf->SetXY(26 + $ac_cell_width,45);
	$pdf->Cell(4,6,$acc_no_arr1[$cells],1,'L');
	$ac_cell_width += 4;
}
...
$cell_width = 0;
for($cells = 0; $cells < 16; $cells++){
	$pdf->SetXY(260 + $cell_width,23);
	$pdf->Cell(6,10,$acc_no_arr1[$cells],1,'L');
	$cell_width += 6;
}
