...
<div class="row mg_bt_10">
    <div class="col-md-12">
        ...
        <table id="tbl_package_hotel_master_dynamic_update" name="tbl_package_hotel_master_dynamic_update" class="table table-bordered no-marg pd_bt_51">
			<?php 
			$sq_plane_count = mysqli_num_rows(mysqlQuery("select * from group_tour_hotel_entries where tour_id='$tour_id'"));
			if($sq_plane_count==0){
				?>
                <tr>
				...
				$offset = "_d";
				$count = 0;
				$sq_q_hotel = mysqlQuery("select * from group_tour_hotel_entries where tour_id='$tour_id'");
				while($row_q_hotel = mysqli_fetch_assoc($sq_q_hotel)){
                    ...
                    <tr>
                        ...
                        <?php $sq_city = mysqli_fetch_assoc(mysqlQuery("select city_name from city_master where city_id='$row_q_hotel[city_id]'")); ?>
                        ...
                        <?php $sq_hotel = mysqli_fetch_assoc(mysqlQuery("select hotel_name from hotel_master where hotel_id='$row_q_hotel[hotel_id]'")); ?>

                        <?php
                        $query = mysqlQuery("select * from hotel_master where city_id = ".$row_q_hotel['city_id']);
                        ...
                        <td><input type="text" id="hotel_type<?= $offset.$count?>_d" name="hotel_type1<?= $offset.$count?>_d" placeholder="*Hotel Category" title="Hotel Category" value="<?php echo $row_q_hotel['hotel_type'] ?>" readonly></td>
