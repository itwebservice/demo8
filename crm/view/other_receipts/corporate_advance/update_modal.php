...
$sq_income_info = mysqli_fetch_assoc(mysqlQuery("select * from corporate_advance_master where advance_id='$advance_id'"));
            ...
            <div class="row">
              <div class="col-md-4 col-sm-6 col-xs-12 mg_bt_10">
                <select id="cust_id" name="cust_id" style="width:100%" title="Customer" disabled>
                  ...
                  $sq_income_type = mysqli_fetch_assoc(mysqlQuery("select * from customer_master where customer_id='$cust_id'"));
                  if($sq_income_type['type'] == 'Corporate'||$sq_income_type['type'] == 'B2B'){
                    $customer_name = $sq_income_type['company_name'];
                  }else{
                    $customer_name = $sq_income_type['first_name'].' '.$sq_income_type['last_name'];
                  }
                  ?>
                  <option value="<?= $sq_income_type['customer_id'] ?>"><?= $customer_name ?></option>
                  <option value="">Select Customer</option>
                  <?php 
                  $sq_cust = mysqlQuery("select * from customer_master where active_flag='Active' ");
