...
$branch_status = $_POST['branch_status'];
		...
		while($row_payment = mysqli_fetch_assoc($sq_payment)){
			if($row_payment['amount']!=0){
				...
				$sq_pay = mysqli_fetch_assoc(mysqlQuery("select sum(amount) as sum from package_payment_master where clearance_status!='Cancelled' and booking_id='$row_payment[booking_id]'"));
