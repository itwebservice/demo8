...
$sq_payment_info = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum from passport_payment_master where passport_id='$passport_id' and clearance_status!='Pending' and clearance_status!='Cancelled'"));

$sq_refund_info = mysqli_fetch_assoc(mysqlQuery("SELECT sum(refund_amount) as sum from passport_refund_master where passport_id='$passport_id' and clearance_status!='Pending' and clearance_status!='Cancelled'"));

$sq_pend_ref= mysqli_fetch_assoc(mysqlQuery("select sum(refund_amount) as sum from passport_refund_master where passport_id='$passport_id' and clearance_status='Pending'"));
...
<table class="table table-bordered table-hover bg_white cust_table" id="tbl_refund" style="margin:20px 0 !important;">
	...
	<tbody>
		...
		while($row_refund = mysqli_fetch_assoc($sq_refund)){
			...
			$sq_entry_info = mysqli_fetch_assoc(mysqlQuery("select * from passport_master where passport_id='$row_refund[passport_id]'"));
			$date = $row_refund['created_at'];
			$yr = explode("-", $date);
			$year1 =$yr[0];
			$traveler_name = "";
			$sq_refund_entries = mysqlQuery("select * from passport_refund_entries where refund_id='$row_refund[refund_id]'");
			while($row_refund_entry = mysqli_fetch_assoc($sq_refund_entries)){
				$sq_entry_info = mysqli_fetch_assoc(mysqlQuery("select * from passport_master_entries where entry_id='$row_refund_entry[entry_id]'"));
