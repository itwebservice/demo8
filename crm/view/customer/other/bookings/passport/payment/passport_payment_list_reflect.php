...
<table class="table table-bordered bg_white cust_table" id="tbl_passport_payment" style="margin:20px 0 !important;">
	...
	<tbody>
		...
		while($row_passport_payment = mysqli_fetch_assoc($sq_passport_payment)){
			...
			$sq_pay = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum,sum(credit_charges) as sumc from passport_payment_master where clearance_status!='Cancelled' and clearance_status!='Pending' and passport_id='$row_passport_payment[passport_id]'"));
