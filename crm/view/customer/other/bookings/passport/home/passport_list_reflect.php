...
<table class="table table-bordered cust_table bg_white" id="tbl_passport_list" style="margin:20px 0 !important;">
	...
	<tbody>
		...
		while($row_passport = mysqli_fetch_assoc($sq_passport)){
			...
			$sq_customer_info = mysqli_fetch_assoc(mysqlQuery("select * from customer_master where customer_id='$row_passport[customer_id]'"));			
			$date = $row_passport['created_at'];
			$yr = explode("-", $date);
			$year =$yr[0];
			$sq_paid_amount = mysqli_fetch_assoc(mysqlQuery("SELECT sum(payment_amount) as sum from passport_payment_master where passport_id='$row_passport[passport_id]' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
			//Get Total no of passport members
            $sq_total_member=mysqli_num_rows(mysqlQuery("select passport_id from passport_master_entries where passport_id='$row_passport[passport_id]' and status!='Cancel'")); 
