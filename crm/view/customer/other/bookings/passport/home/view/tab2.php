<?php 
//sale
$sq_credit = mysqli_fetch_assoc(mysqlQuery("SELECT sum(`credit_charges`) as sumc FROM `passport_payment_master` WHERE `passport_id`='$sq_passport_info[passport_id]' and clearance_status != 'Pending' and `clearance_status`!='Cancelled'"));
...
$pass_count = mysqli_num_rows(mysqlQuery("select * from  passport_master_entries where passport_id='$sq_passport_info[passport_id]'"));
$cancel_count = mysqli_num_rows(mysqlQuery("select * from  passport_master_entries where passport_id='$sq_passport_info[passport_id]' and status='Cancel'"));

//Paid
$query = mysqli_fetch_assoc(mysqlQuery("SELECT sum(payment_amount) as sum,sum(credit_charges) as sumc from passport_payment_master where passport_id='$passport_id' and clearance_status != 'Pending' and clearance_status != 'Cancelled'"));
				...
				<div class="table-responsive">
					<table class="table table-bordered no-marg">
						...
						<tbody>
							...
							$bg;
							$sq_passport_payment = mysqlQuery($query);
							$sq_pending_amount = 0;
							$sq_cancel_amount = 0;
							$sq_paid_amount = 0;
							$count = 0;

							while($row_passport_payment = mysqli_fetch_assoc($sq_passport_payment)){
								if($row_passport_payment['payment_amount'] != '0'){
									...
									$sq_passport_info = mysqli_fetch_assoc(mysqlQuery("select * from passport_master where passport_id='$row_passport_payment[passport_id]'"));
