...
<table class="table table-bordered bg_white cust_table" id="ticket_list" style="margin:20px 0 !important">
	...
	<tbody>
		...
		while($row_ticket = mysqli_fetch_assoc($sq_ticket)){
			...
			$sq_ticket_info = mysqli_fetch_assoc(mysqlQuery("select * from ticket_trip_entries where ticket_id='$row_ticket[ticket_id]'"));

			$sq_paid_amount = mysqli_fetch_assoc(mysqlQuery("SELECT sum(payment_amount) as sum from ticket_payment_master where ticket_id='$row_ticket[ticket_id]' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
