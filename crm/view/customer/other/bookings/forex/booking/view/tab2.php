...
$cancel_amount=0;
$query = mysqli_fetch_assoc(mysqlQuery("SELECT sum(payment_amount) as sum,sum(credit_charges) as sumc from forex_booking_payment_master where booking_id='$booking_id' and clearance_status != 'Pending' and clearance_status != 'Cancelled'"));
                    ...
                    <table class="table table-bordered no-marg" id="tbl_list">
						...
						while($row_payment = mysqli_fetch_assoc($sq_payment)){
								if($row_payment['payment_amount'] != '0'){
										...
										$count++;
										$sq_bus_info = mysqli_fetch_assoc(mysqlQuery("select * from forex_booking_master where booking_id='$row_payment[booking_id]'"));
