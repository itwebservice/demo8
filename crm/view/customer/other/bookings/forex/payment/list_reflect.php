...
<table class="table table-bordered cust_table" id="tbl_list_f1" style="margin:20px 0 !important;">
	...
	<tbody>
		...
		while($row_payment = mysqli_fetch_assoc($sq_payment)){
			...
			$year =$yr[0];
			$count++;
			$sq_forex_info = mysqli_fetch_assoc(mysqlQuery("select * from forex_booking_master where booking_id='$row_payment[booking_id]'"));
			$total_sale = $sq_forex_info['net_total'];
			$sq_pay = mysqli_fetch_assoc(mysqlQuery("select sum(payment_amount) as sum from forex_booking_payment_master where clearance_status!='Cancelled' and booking_id='$row_payment[booking_id]'"));
			...
			$yr = explode("-", $date);
			$year1 =$yr[0];
			$sq_customer_info = mysqli_fetch_assoc(mysqlQuery("select * from customer_master where customer_id='$sq_forex_info[customer_id]'"));
