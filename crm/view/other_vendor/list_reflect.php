...
<table class="table table-bordered" id="tbl_list" style="margin: 20px 0 !important;">
	...
	<tbody>
		<?php 
		$count = 0;
		$sq_vendors = mysqlQuery($query);
		while($row_vendors = mysqli_fetch_assoc($sq_vendors)){
			$sq_city = mysqli_fetch_assoc(mysqlQuery("select city_name from city_master where city_id='$row_vendors[city_id]'"));
