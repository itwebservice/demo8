        ...
        <div class="panel panel-default panel-body app_panel_style feildset-panel mg_tp_30">
            ...
            <div class="row mg_bt_10">
              ...
              <small id="service_show" style="color:#000000"><?= ($inclusive_s == '') ? '&nbsp;' : 'Inclusive Amount : <span>'.$inclusive_s ?></span></small>
                <input type="text" id="service_charge" name="service_charge" placeholder="Service Charge" title="Service Charge"  onchange="calculate_total_amount();validate_balance(this.id);get_auto_values('balance_date1','basic_cost','payment_mode','service_charge','markup','update','true','service_charge');" value="<?= $service_charge ?>">
              </div>
...
$('#journey_date').datetimepicker({ format:'d-m-Y H:i' });
...
$('#frm_update').validate({
    ...
    submitHandler:function(){
            ...
            $('#btn_update').prop('disabled', false); return false; }
            ...
            for(var i=0; i<rowCount; i++)
            {
              ...
              if(row.cells[0].childNodes[0].checked)
              {
                  ...
                  if(company_name == ''){
                    error_msg_alert("Enter Bus Operator name at row "+(i+1));$('#btn_update').prop('disabled', false);
                    return false;
                  }
                  if (origin == '') {
                    error_msg_alert("Enter Source at row " + (i + 1));$('#btn_update').prop('disabled', false);
                    return false;
                  }
                  if (destination == '') {
                    error_msg_alert("Enter Destination at row " + (i + 1));$('#btn_update').prop('disabled', false);
                    return false;
                  }
