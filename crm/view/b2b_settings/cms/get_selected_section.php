...
elseif ($section_name == '2') {
    $why_choose_us = ($query['why_choose_us']!='' && $query['why_choose_us']!='null') ? json_decode($query['why_choose_us']): [];
?>
...
<?php }
    if($section_name == '3'){
    $amazing_dest_ideas = ($query['amazing_dest_ideas']!='' && $query['amazing_dest_ideas']!='null') ? json_decode($query['amazing_dest_ideas']): []; ?>
...
elseif ($section_name == '5') {
    ...
    $popular_honey_hotels = ($query['popular_honey_hotels']!='' && $query['popular_honey_hotels']!='null') ? json_decode($query['popular_honey_hotels']): [];
    $hotels = ($popular_honey_hotels[0]->hotel!='')?$popular_honey_hotels[0]->hotel:[];
?>
    ...
    if(sizeof($hotels)>0){
        for($i=0;$i<4;$i++){
            ...
            <div class="row mg_bt_20">
                <div class="col-md-3">
                    <select id='city_name-<?= $i+1 ?>' title="Select City" class="form-control city_id_cms" onchange="hotel_names_load(this.id);city_lzloading('.city_id_cms')">
                    <?php if($city_id!=''){ ?>
...
elseif ($section_name == '6') {
    ...
    $popular_activities = ($query['popular_activities']!='' && $query['popular_activities']!='null') ? json_decode($query['popular_activities']): [];
?>
...
elseif ($section_name == '7') {
    ...
    $call_to_action = ($query['call_to_action']!='' && $query['call_to_action']!='null') ? json_decode($query['call_to_action']): [];
?>
...
elseif ($section_name == '9') {
    $popular_honey_dest = ($query['popular_honey_dest']!='' && $query['popular_honey_dest']!='null') ? json_decode($query['popular_honey_dest']): [];
    $destination = ($popular_honey_dest[0]->destination!='')?$popular_honey_dest[0]->destination:[];
?>
...
elseif ($section_name == '10') {
    $col1 = ($query1['col1']!='' && $query1['col1']!='null') ? json_decode($query1['col1']): [];
    $col2 = ($query1['col2']!='' && $query1['col2']!='null') ? json_decode($query1['col2']): [];
    $col3 = ($query1['col3']!='' && $query1['col3']!='null') ? json_decode($query1['col3']): [];
?>
