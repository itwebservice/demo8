...
if($travel_type=='Local'){
    $sq = mysqlQuery("select * from car_rental_tariff_entries where vehicle_name='$vehicle_name' and tour_type = '$travel_type' and status!='Inactive'");
}else{
    $sq = mysqlQuery("select * from car_rental_tariff_entries where vehicle_name='$vehicle_name' and tour_type = '$travel_type' and route = '$places_to_visit' and status!='Inactive'");
}

    while($sql_entries = mysqli_fetch_assoc($sq)){
