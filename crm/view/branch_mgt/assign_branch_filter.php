...
<h4>Branchwise Permissions</h4>
		...
		foreach($xml->menu_desc as $value)
		{
			...
			$assigned_role = mysqli_num_rows(mysqlQuery("select * from branch_assign where name='$name' and link='$link' and branch_status='yes'"));
			if($assigned_role>=1){
				$status = "checked";
			}
			else{
			...
			$disabled_status =mysqlQuery("select * from branch_assign where link='$link' and branch_status='disabled' ");
			while($row=mysqli_fetch_assoc($disabled_status)){
				$link_d = $row['link'];
			}
			if($link==$link_d){
