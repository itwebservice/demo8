...
<div class="header_bottom mg_tp_10">
	<div class="row">
		<div class="col-sm-6">
			<button class="btn btn-info btn-sm ico_left pull-left" title="Download CSV Format" style="margin-right:10px" onclick="display_format_modal();"><i class="fa fa-download"></i>&nbsp;&nbsp;CSV Format</button>
		...
		<div class="col-md-12">
			<div class="app_panel_content Filter-panel">
				...
				elseif($branch_status=='yes' && $role=='Branch Admin'){  ?>
					...
					<div class="col-md-3 col-sm-6 mg_bt_10">
						<input type="text" id="followup_from_date_filter" name="followup_from_date_filter" placeholder="From Date" title="From Date" onchange="get_to_date(this.id,'followup_to_date_filter')">
					</div>
					<div class="col-md-3 col-sm-6 mg_bt_10">
						<input type="text" id="followup_to_date_filter" name="followup_to_date_filter" placeholder="To Date" title="To Date" onchange="validate_validDate('followup_from_date_filter','followup_to_date_filter')">
					</div>
				</div>
				<div class="row">
...
var columns = [
	...
	{ title: "Allocate_To",className:"action_width"},
...
function enquiry_proceed_reflect(){
	...
	method:"POST",
	data:{enquiry:enquiry,enquiry_type : enquiry_type, enquiry_status : enquiry_status, from_date : from_date, to_date : to_date, emp_id_filter : emp_id_filter , branch_status : branch_status, reference_id_filter:reference_id_filter},
	cache:false,
	success:function(data){
		// console.log(data);
		setTimeout(() => {
...
function view_modal(enquiry_id)
{
	$.post('view_modal.php', { enquiry_id : enquiry_id }, function(data){
		$('#div_modal').html(data);console.log(data)
	});
}
