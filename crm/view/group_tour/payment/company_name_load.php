...
<select name="company_filter" style="width: 100%" id="company_filter" onchange="dynamic_customer_load('cust_type_filter',this.value);" title="Select Company">
      ...
      <?php 
            if($role=='Admin') {
                  $sq_query = mysqlQuery("select distinct(company_name) from customer_master where (type = 'Corporate' or type = 'B2B') order by company_name");
            ...
            if($branch_status=='yes' && $role!='Admin'){
                  ...
                  $sq_query = mysqlQuery("select distinct(company_name) from customer_master where (type = 'Corporate' or type = 'B2B') and branch_admin_id='$branch_admin_id' order by company_name");
                  while($row_query=mysqli_fetch_assoc($sq_query)){ ?>
                              <option value="<?php echo $row_query['company_name']; ?>"><?php echo $row_query['company_name']; ?></option>
            <?php   } 
                  }
