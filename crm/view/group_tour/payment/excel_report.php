...
while($row_payment = mysqli_fetch_assoc($sq_payment)){
      ...
      $total = $total+$row_payment['amount']+$row_payment['credit_charges'];
        ...
        ->setCellValue('I'.$row_count, number_format($row_payment['amount']+$row_payment['credit_charges'],2).$currency_amount);
...
header('Content-Disposition: attachment;filename="Group Receipt('.date('d-m-Y H:i').').xls"');
