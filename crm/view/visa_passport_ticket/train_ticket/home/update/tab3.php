...
<?php
	...
	<div class="row">
		...
		<div class="col-md-2 col-sm-4 col-xs-12 mg_bt_10">
			...
			<input type="text" id="service_charge" name="service_charge" placeholder="Service Charge" title="Service Charge" onchange="get_auto_values('booking_date1','basic_fair','payment_mode','service_charge','markup1','update','true','service_charge','discount1');calculate_total_amount();validate_balance(this.id)" value="<?= $service_charge ?>">
		</div>
		<div class="col-md-2 col-sm-4 col-xs-12 mg_bt_10">
            <small>&nbsp;</small>
	...
	<div class="row text-center mg_tp_20">
		...
		<div class="col-xs-12">
			...
			<button class="btn btn-sm btn-success" id="btn_ticket_update"><i class="fa fa-floppy-o"></i>&nbsp;&nbsp;Update</button>
...
$('#frm_tab3').validate({
	...
	submitHandler:function(form){		
		...
		$('#btn_ticket_update').prop('disabled',true);
		...
		$('#btn_ticket_update').prop('disabled',false); return false; }
			...
			$.post(base_url+'view/load_data/finance_date_validation.php', { check_date: check_date1 }, function(data){
				if(data !== 'valid'){
					...
					return false;
						...
						$('#btn_ticket_update').button('loading');
						$.ajax({
							...
							success:function(result){
								...
								$('#btn_ticket_update').button('reset');

								var msg = result.split('--');

								if(msg[0]=="error"){

									errormsg_alert(result);
									$('#btn_ticket_update').prop('disabled',false);
									return false;
								}
