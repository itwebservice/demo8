...
$sq_passenger = mysqlQuery("select * from  train_ticket_master_entries where train_ticket_id = '$train_ticket_id'");
...

	{
		...
		$sq_dest = mysqli_fetch_assoc(mysqlQuery("select * from train_ticket_master_trip_entries where train_ticket_id = '$row_passenger[train_ticket_id]'"));

	    $pdf->SetX(10);

	    $pdf->Row(array($count,$row_passenger['first_name'].' '.$row_passenger['last_name'],$sq_dest['travel_from'], $sq_dest['travel_to'],  date("d-m-Y H:i", strtotime($sq_dest['travel_datetime'])),$row_passenger['seat_number'],$row_passenger['ticket_number']));
