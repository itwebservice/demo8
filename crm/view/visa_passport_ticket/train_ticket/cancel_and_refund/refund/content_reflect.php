...
$sq_pay = mysqli_fetch_assoc(mysqlQuery("select sum(refund_amount) as sum from train_ticket_refund_master where train_ticket_id='$sq_train_ticket_info[train_ticket_id]'"));
...
$sq_ref_can_total = mysqli_fetch_assoc(mysqlQuery("select sum(refund_amount) as sum from train_ticket_refund_master where train_ticket_id='$sq_train_ticket_info[train_ticket_id]' and clearance_status='Cancelled'"));
		...
		<table class="table table-bordered" id="tbl_refund_list" style="margin: 0 !important;">
			...
			<tbody>
				...
				while($row_train_ticket_refund = mysqli_fetch_assoc($sq_train_ticket_refund)){
					...
					else if($row_train_ticket_refund['clearance_status']=='Cleared'){ $bg = "success"; }
					else{ $bg = ""; }
			...
			<tfoot>
				...
				<tr class="active">
					...
					<input type="hidden" name="remaining_val" id="remaining_val" value="<?php echo $refund_amount-($total_refund-$sq_refund_can_amount['sum']);?>">
				</tr>
			</tfoot>
		</table>
		</div> 
