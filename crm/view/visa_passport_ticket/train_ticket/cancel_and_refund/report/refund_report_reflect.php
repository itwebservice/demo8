...
<table class="table table-hover" id="tbl_refund" style="margin: 20px 0 !important;">
	...
	<tbody>
		...
		while($row_refund = mysqli_fetch_assoc($sq_refund)){
			...
			<tr class="<?= $bg ?>">
				...
				<td><?= $traveler_name ?></td>
				<td><?= get_train_ticket_booking_refund_id($row_refund['refund_id'],$year1); ?></td>
	...
	<tfoot>
		<tr class="active">
			<th colspan="3" class="text-right info">Refund : <?= number_format($total_refund,2)?></th>
			...
			<th colspan="2" class="text-right success">Total_Refund : <?= number_format(($total_refund - $sq_pending_amount - $sq_cancel_amount),2)  ?></th>
		</tr>
	</tfoot>
</table>
