	                       		...
	                       		while($row_entry = mysqli_fetch_assoc($sq_entry)){
						            ...
						            }else
						            {
									 ...
									 <tr class="<?= $bg ?>">
									    ...
									    <td><select name="visa_type<?= $offset ?>1" id="visa_type<?= $offset ?>1" title="Visa Type" onchange="reflect_cost(this.id)">
									    ...
									    <td><input type="text" id="expiry_date<?= $offset ?>1"  name="expiry_date<?= $offset ?>1" class="app_datepicker" placeholder="Expiry Date" title="Expiry Date" value="<?=  get_date_user($row_entry['expiry_date']) ?>" onchange="validate_issueDate('issue_date<?= $offset ?>1',this.id)"></td>
									...
									<script>
										$("#birth_date<?= $offset.$count ?>_d, #issue_date<?= $offset ?>1").datetimepicker({ timepicker:false, format:'d-m-Y' });
...
function reflect_cost(id){
	...
	$.post('<?= BASE_URL?>view/visa_passport_ticket/visa/home/visa_cost_reflect.php', {visa_type : visa_type, visa_country : visa_country}, function(data){
		...
		$('#visa_cost'+offset).val(parseFloat(amount)+parseFloat(service));
