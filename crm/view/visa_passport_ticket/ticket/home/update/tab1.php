	...
	<div class="panel panel-default panel-body app_panel_style feildset-panel">
         <legend>Quotation Details</legend>
          <div class="row">
            <div class="col-md-4">
			<?php
				$customer_name = mysqli_fetch_assoc(mysqlQuery("SELECT * FROM `flight_quotation_master` WHERE quotation_id = ".$sq_ticket['quotation_id']));
				$quotation_date = $customer_name['quotation_date'];
                $yr = explode("-", $quotation_date);
                $year =$yr[0];
			?>
			<select name="quotation_id" id="quotation_id" style="width:100%" class="form-control" disabled>
			<?php
				if($sq_ticket['quotation_id'] == '0'){
			?>
				<option>Quotation Not selected</option>
			<?php
			}
			?>
			<option value="<?= $sq_ticket['quotation_id'] ?>"><?= get_quotation_id($sq_ticket['quotation_id'],$year )." ".$customer_name['customer_name'] ?></option>
			</select>
              </div>
          </div>
        </div>
