<form id="frm_tab2">
	<div class="row">
		...
		<div class="col-md-4 col-sm-12 col-xs-12 text-right">
			<button type="button" class="btn btn-info btn-sm ico_left" onclick="addDyn('div_dynamic_ticket_info');event_airport_su('','1');copy_values()"><i class="fa fa-plus"></i>&nbsp;&nbsp;Add Section</button>
		</div>
	</div>
	<?php
				...
				<div class="dynform-item">
					...
					<div class="row">
						...
						<div class="col-md-4 col-sm-12 col-xs-12">
							<textarea name="special_note" onchange="validate_address(this.id) ;" id="special_note-<?= $count ?>" rows="1" placeholder="Operated by" title="Operated by" data-dyn-valid="required"><?= $row_entry['special_note'] ?></textarea>
						</div>
					</div>
				</div>
				<script>
...
function event_airport_su(count = 2,add_row=''){
	...
	if(count == 1)	{
		...
		count = (add_row!='') ? $('#div_dynamic_ticket_info').attr('data-counter') : count;
		id1 = "airpf-"+count;
		id2 = "airpt-"+count;
