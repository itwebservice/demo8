...
for($i = 0; $i < sizeof($entryidArray) + $addIterator; $i++){
    $passengers = mysqli_fetch_assoc(mysqlQuery("SELECT * FROM `ticket_master_entries_airfile` WHERE `entry_id` = ".$entryidArray[$i]));
...
foreach($ticketairfileID as $values){
    $sectInfo = mysqlQuery("SELECT * FROM `ticket_trip_entries_airfile` WHERE `ticket_airfile_id` = ".$values);
    while($rows = mysqli_fetch_assoc($sectInfo)){
        $fromCity = mysqli_fetch_assoc(mysqlQuery("SELECT `city_name` FROM `city_master` WHERE `city_id` =".$rows['from_city']));
        $rows['from_city_show'] = $fromCity['city_name'];
        $toCity = mysqli_fetch_assoc(mysqlQuery("SELECT `city_name` FROM `city_master` WHERE `city_id` =".$rows['to_city']));
...
foreach($uniquePnr as $values){
    $ticketEntries = mysqli_fetch_assoc(mysqlQuery("SELECT * FROM `ticket_master_entries_airfile` WHERE `gds_pnr` = '".$values."'"));
    $ticketMaster =  mysqli_fetch_assoc(mysqlQuery("SELECT * FROM `ticket_master_airfile` WHERE `ticket_airfile_id` = ".$ticketEntries['ticket_airfile_id']));
