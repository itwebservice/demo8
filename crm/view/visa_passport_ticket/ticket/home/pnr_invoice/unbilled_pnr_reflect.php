...
$pnr = mysqlQuery("SELECT * FROM `ticket_master_entries_airfile` WHERE `status` = ''");
$count = 1;
$array_s = array();
$temp_arr = array();
// $unsavedAirp = '';
while($rows = mysqli_fetch_assoc($pnr)){
    $masterTable = mysqli_fetch_assoc(mysqlQuery("SELECT * FROM `ticket_master_airfile` WHERE `ticket_airfile_id` = ".$rows['ticket_airfile_id']));
    $couponTable = mysqlQuery("SELECT * FROM `ticket_trip_entries_airfile` WHERE `ticket_airfile_id` = ".$rows['ticket_airfile_id']);
    $trip = '';$travelDate = array();
    while($rowCoupontable = mysqli_fetch_assoc($couponTable)){
