function train_ticket_id_dropdown_load(customer_id_filter, train_ticket_id_filter) {
	...
	$.post('ticket_id_dropdown_load.php', { customer_id: customer_id, branch_status: branch_status }, function (data) {
		...
		$('#'+train_ticket_id_filter).val('');
        $('#'+train_ticket_id_filter).trigger('change');
	});
}
...
function whatsapp_send(emp_id, customer_id, booking_date, base_url, contact_no,email_id) {
	$.post(base_url + 'controller/visa_passport_ticket/train_ticket/whatsapp_send.php', { emp_id: emp_id, booking_date: booking_date, customer_id: customer_id, booking_date: booking_date,contact_no:contact_no,email_id:email_id }, function (data) {
		window.open(data);
	});
}