...
$sq_booking = mysqli_fetch_assoc( mysqlQuery("select traveler_group_id from tourwise_traveler_details where id='$booking_id'") );
...
<legend>Upload Ticket</legend>
	...
	<select name="traveler_id" id="traveler_id" multiple size="5">
		<?php 
			$sq_traveler = mysqlQuery("select * from travelers_details where traveler_group_id='$traveler_group_id' and status!='Cancel' and traveler_id not in ( select traveler_id from travelers_tickets_entries where tourwise_traveler_id='$booking_id' )");
...
<legend>Download Ticket</legend>
	...
	<table>
		...
		<?php 
		$count = 0;
		$sq_ticket = mysqlQuery("select * from travelers_tickets_master where tourwise_traveler_id='$booking_id'");
		while($row_ticket = mysqli_fetch_assoc($sq_ticket))
		{
			$ticket_id = $row_ticket['ticket_id'];
			$ticket_url = preg_replace('/(\/+)/','/',$row_ticket['ticket_url']);                
			$sq_ticket_entries_count = mysqli_num_rows( mysqlQuery("select * from travelers_tickets_entries where ticket_id='$ticket_id'") );
			?>
			<?php
			$fisrt_time = true;
			$sq_ticket_entries = mysqlQuery("select * from travelers_tickets_entries where ticket_id='$ticket_id'");
			while($row_ticket_entries = mysqli_fetch_assoc($sq_ticket_entries))
			{
				$traveler_id = $row_ticket_entries['traveler_id'];
				$sq_traveler_det = mysqli_fetch_assoc(mysqlQuery("select m_honorific, first_name, last_name from travelers_details where traveler_id='$traveler_id'"));
